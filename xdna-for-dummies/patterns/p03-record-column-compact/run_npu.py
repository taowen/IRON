#!/usr/bin/env python3
"""NPU runner for P3: record column compact."""
...
import generate
import npu_build
from reference import CASE_NAME, COMPACT_DWORDS, expected_output, validate_output
...
def check_only() -> bool:
    ...
    if errors:
        for e in errors:
            print(f"  FAIL: {e}")
        return False
    print("  PASS: MLIR structure valid (2 producers -> memtile compact -> host)")
    return True


def run() -> bool:
    print("=" * 70)
    print("Pattern 3: Record → Column Compact")
    print("=" * 70)
    print("  prod0(2,2) ──► shim0 ──►╮")
    print("                           ├── host output BO (compacted)")
    print("  prod1(3,2) ──► shim1 ──►╯")
    print(f"  Each producer emits {generate.RECORD_DWORDS} dwords (1 header + {generate.PAYLOAD_DWORDS} payload)")
    print(f"  Compact output = {COMPACT_DWORDS} dwords (both records concatenated)")
    print()
    ...
    output_buf = XRTTensor((COMPACT_DWORDS,), dtype=np.uint32)
    ...
    print("  PASS: 2 producer records compacted into single output by memtile")
    return True
