#!/usr/bin/env python3
"""NPU runner for P10: fusion ABI handoff."""
...
import generate
import npu_build
from reference import CASE_NAME, DATA_DWORDS, RECORD_DWORDS, make_input, validate_output
...
def check_only() -> bool:
    ...
    if errors:
        for e in errors:
            print(f"  FAIL: {e}")
        return False
    print("  PASS: MLIR structure valid (host -> A -> B -> host, no intermediate BO)")
    return True


def run() -> bool:
    print("=" * 70)
    print("Pattern 10: Fusion = Stable ABI Handoff")
    print("=" * 70)
    print("  host → tile_a(emit record: header+payload) → tile_b(consume record) → host")
    print("  ABI: 17-dword record = 1 header (op_id|count) + 16 payload")
    print("  Intermediate record stays ON-CHIP (no host buffer)")
    print()
    ...
    output_buf = XRTTensor((RECORD_DWORDS,), dtype=np.uint32)
    ...
    print(f"  header: got=0x{got[0]:08X} expected=0x{expected[0]:08X}")
    print(f"  payload[0:4]: got={got[1:5].tolist()} expected={expected[1:5].tolist()}")
    print(f"  source_op_id: got=0x{got[DATA_DWORDS]:02X} expected=0x{expected[DATA_DWORDS]:02X}")
    ...
    print("  PASS: record ABI handoff on-chip, header+payload verified")
    return True
