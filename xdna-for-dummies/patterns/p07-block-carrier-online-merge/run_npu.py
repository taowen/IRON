#!/usr/bin/env python3
"""NPU runner for P7: block carrier + online merge."""
...
import generate
import npu_build
from reference import CASE_NAME, INPUT_DWORDS, OUTPUT_DWORDS, make_input, pack_input, validate_output
...
def check_only() -> bool:
    ...
    if errors:
        for e in errors:
            print(f"  FAIL: {e}")
        return False
    print("  PASS: MLIR structure valid (block → carrier → online merge)")
    return True


def run() -> bool:
    print("=" * 70)
    print("Pattern 7: Block Carrier + Online Merge (weighted mean)")
    print("=" * 70)
    print(f"  {generate.NUM_BLOCKS} blocks of (scores[{generate.BLOCK_DWORDS}], values[{generate.BLOCK_DWORDS}])")
    print(f"  Carrier: 3 dwords = (block_max, block_sum, weighted_value)")
    print(f"  Online merge: rescale running state when new block has higher max")
    print(f"  Output: weighted_mean = running_wv / running_sum")
    print()
    ...
    scores, values = make_input()
    input_data = pack_input(scores, values)
    ...
    print(f"  expected: mean={expected[0]}, max={expected[1]}, sum={expected[2]}")
    print(f"  got:      mean={got[0]}, max={got[1]}, sum={got[2]}")

    errors = validate_output(got, scores, values)
    if errors:
        for e in errors:
            print(f"  FAIL: {e}")
        return False

    print("  PASS: online weighted-mean merge with rescaling works correctly")
    return True
