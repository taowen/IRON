...
- **性能路线**：宏观数据流继续按 MyLM-style fused layer engine 走；后续不是推翻 row1/c1r1/c6r1/topology，而是把当前 linked C++ scheduler + source-asm Q4NX hot body 继续压缩成 MyLM-style raw/scheduled core body。
...
- IRON 当前 main16 已经不是大段 MLIR phase control。active MLIR core 统一调用 `func.call @q4nx_main16_layer_scheduler(..., phase_limit)`，该函数通过 `link_with` 链接进每个 main16 tile 的 C++/source-asm AIE core object，作为 tile-local core program entry 实现 phase dispatch。slice 不再使用 QKV/QKVO/full 三套 scheduler：QKV prefix 传 `phase_limit=3`，attention-O 传 `4`，full layer 传 `7`。fresh full-decode `main_core_2_2.elf` 反汇编包含 active `q4nx_chunk_accum_asm_zol` hot body。因此旧的“MLIR 展开 phase loop”问题已经基本迁移到 linked AIE core kernel 中。
- 最新差距集中在 Q4NX microkernel 指令形态：MyLM `0x1f0..0x1850` hot loop 有 `vmac.f=264`、`vextbcst.16=256`、`vunpack=64`、`vups.4x=64`、`vconv.bf16.fp32=136`、`vst=0`。它不是 8 个独立 33-MAC group，而是 `fill(group0) + steady_template(group1)*5 + pre_drain(group6) + drain(group7)` 的跨 group 软件流水；group1..5 的 instruction text 完全一致，group6 在尾部加入 `p3` scratch rewind，group0/group7 分别负责 fill/drain。exp107 已经能用这四个模板 exact replay MyLM `0x260..0x1850` 的 `1532` 个 instruction slots，所以现在掌握的是可生成 schedule，不只是局部 opcode。exp108 确认 scaled-only group-sum 不是当前 exact reference 的静默替换：layer35/token0 真实 dump hidden 下 `hidden_out max_abs=44.0`。exp109 进一步把根因缩小到公式形态：`centered = bf16(bf16(q*scale)+zero)-zero` 再加 `zero*sum(activation_group)` 可以把同一输入的 `hidden_out max_abs` 降到 `0.0625`，只剩 2 个 `>1e-2` 的 contraction-order residual。exp110 又确认 centered coefficient 本身不丢精度：`centered+zero` 在 MAC 前重组后 layer35 `hidden_out max_abs=0`；但把 zero 作为单独 accumulator contribution，即使 per-dim 加，也会漂到 `max_abs=0.125`、`>1e-2=10`。所以后续四模板 schedule 有两个明确分支：exact 路线必须在 MAC 前重组 dequant coefficient；MyLM-like `32+1 MAC/group` 路线则是显式数值 contract 变化，需要多层 token gate。exp111 又把动态成本算清楚：active exact 已经是正确的 `512` dynamic MAC，不该继续追 MAC count；真正差距是 coefficient construction/control traffic，active exact `vconv.bf16.fp32=1280`、`vconv.fp32.bf16=768`、`vmul.f=512`、`crupsmode=128`、`nop=3088`，MyLM 分别是 `272/0/16/0/216`。IRON 当前 production hot body 已从普通 AIE API MAC 迁到 source-assembly aligned-ZOL exact lane：`main_projection_q4nx_fast.cc` 的 scheduler 直接调用 `q4nx_chunk_accum_asm_zol`，旧 `q4nx_chunk_accum_fast` C++ wrapper 已从 active object 删除。该 asm body 静态形态是 `vmac.f=64`、`vextbcst.16=64`、`vextbcst.32=0`、`vunpack=64`、`vups.2x=32`、`vlda=2`、`vldb=66`、`vst=2`，并通过 `main16-q4nx-compute-perf` 和 full decode 数值门；但 `vconv.bf16.fp32=160`、`vmul/vadd` dequant 路径和 call-state 保存/恢复仍重。最新可运行优化把 activation group load 合并到每 32-dim 一次，并把 accumulator handoff latency 从 12 个 nop 收到 6 个 nop；`main16-q4nx-compute-perf=18.158 ms`，QKV prefix token31 `6.572 ms`，full decode token31 `28.479 ms`。所以 asm ZOL 已经是唯一 active main16 hot body，并且小步优化有效；但它仍慢于旧 C++ path 约 `13.6 ms`，5x 级性能差距现在主要不是 Python generator 或 MLIR loop annotation，而是 active exact asm lane 仍没有达到 MyLM raw scheduled unpack/dequant/MAC 密度。
- exp112/113/114/115/116/117/118/119/120/121/122 把下一步边界又收窄了一层：exact parity 的理论下界仍要 `512` 次 vector multiply 和 `1536` 次 conversion，而 MyLM-like group-correction contract 只要 `16` 次 vector multiply 和 `272` 次 conversion；MyLM steady group1 的 coarse `vmac.f` operand trace 显示 `6/66` 个 vector operand 和 `2/33` 个 accumulator source 是跨 group carry；half-register 版本进一步显示 `25/66` 个 vector operand 是 mixed-half，`9/66` 个 vector operand 有跨 group cell。exp102 已经在真机证明 `vmac.f #0x33c` 的 primitive lane model 成立，所以“继续学习汇编”的具体对象不是再认几个 opcode，也不是重新证明 MAC 指令本身，而是把 MyLM 四模板 schedule 反编译成带 half-register 边界状态的 operand graph。exp115 已经输出 Markdown+JSON：`fill -> steady` 有 23 个 incoming data cells，groups `2..6` 的 steady-to-steady data-cell signature 稳定为 `5121c8604a8461c4`。exp116 证明这个 JSON 已能驱动 steady codegen：生成带 half-register 注释的 steady `.s.inc` 后，剥离注释能 exact recover MyLM group1（`189` slots、`33` `vmac.f`、0 个缺失 graph record）。exp117 又把入口扩到完整 hot loop：`fill + steady*5 + pre_drain + drain` 生成物剥离注释后 exact match MyLM `0x260..0x1850`（`1532` slots、hash 相同、`165` 条 steady `vmac.f` 注释来自 graph）。exp118 进一步把 full hot loop 展成 cell liveness：每个 group 边界都稳定跨越 `27` 个 data cell 和 `12` 个 pointer/scalar cell。exp119 把 operand graph 扩到全部 `264` 条 `vmac.f`：group MAC count 是 `28,33,33,33,33,33,33,38`，group1 是 fill-to-steady signature，groups `2..5` 是稳定 steady-to-steady signature。exp120 把这些 artifact 合并成 5 个 section 的 generator contract：`fill/fill_to_steady/steady_to_steady/pre_drain/drain` 宏重新展开后 exact-match 原始 `1532` slots，并保留 `264` group MACs。exp121 用这个 contract 做 modified body 数值分支 gate：`exact_recompose_coeff` 在 nominal/stress synthetic gate 都是 0 差异，MyLM-like group correction 保留 `264/528` MAC shape 但 stress 下有 `>1e-2` mismatch。exp122 将 production-safe 分支机械化：保留 exp120 live-state section，删除每个 logical group 的 1 个 zero-correction MAC，目标 section MAC shape 从 `28,33,33,33,33,33,33,38` 变成 `27,32,32,32,32,32,32,37`，总计 exact `256` static / `512` dynamic MACs。接受 MyLM-like 数值 contract 必须另走真实多层 token gate。
...
- AIEVec/XLLVM 路线可以作为 raw binary 前的中间层。`qwen3-layer/tools/probe_aievec_q4nx_codegen.py` 已确认 `aievec.matmul` 能降到 `xllvm.intr.aie2p.I512.I512.ACC2048.mac.conf`，`aievec.ups/srs` 能降到 BF16 accumulator conversion；但当前 AIEVec/XLLVM op 面只有 `cast/ext/matmul/shift/shuffle/srs/ups`，没有直接暴露 MyLM hot loop 需要的完整 `vunpack/vextbcst.16` 组合。最新 active 决策已经落到 source assembly：`qwen3-layer/main_projection_q4nx_fast.cc` 保留唯一 main16 role scheduler，`qwen3-layer/main_projection_q4nx_asm.s` 提供 active `q4nx_chunk_accum_asm_zol` hot body，二者链接成同一个 `main_projection_q4nx_fast.o`。exp99 的关键规则仍然成立：source-assembly ZOL 必须按 AIE2P 约束生成 16-byte bundle-aligned `ls/le`，并保证 setup 到 loop-end label 至少 112 bytes；scale/offset/activation 用 pointer register 推进，不能用 loop-carried `dj0` offset。迁入后 `tools/check_main16_asm_integration.py --strict` 会检查生成器漂移、final ELF 中的 active asm symbol、坏跳转、坏 lane 残留、旧 C++ wrapper 残留和 `vmac.f/vextbcst.16/ZOL` 指令形态。最新结果是数值正确且比初始 asm 版本更快：`main16-q4nx-compute-perf=18.158 ms`，QKV prefix token31 `6.572 ms`，full decode token31 `28.479 ms`。因此下一步不再是“能不能把 asm 接进来”，而是继续压缩 active asm body 的 dequant/conversion/call-state 开销，向 MyLM 的 raw scheduled loop 靠近。
...
所以接下来不是再调 MLIR loop annotation，也不是继续堆 C++ kernel 变种。工程路线是：保留 MLIR-AIE 生成的 topology/BD/lock/route/runtime/package，保留当前 linked AIE core kernel ABI，并继续优化已经接入 active path 的 source-assembly `q4nx_chunk_accum_asm_zol`。AIEVec/XLLVM 仍可用于独立 probe，但 active main16 不再同时维护 AIEVec/C++/asm 多套 hot body。若 source assembly 仍无法逼近 MyLM 指令密度，再走 raw ET_EXEC ELF 生成完整 main16 core program。
...
`28.479 ms`，所以“4-5x 提升”不是夸张目标，而是至少要追上本机 MyLM probe
才成立。
...
- **只替换 Q4NX microkernel**：保留 IRON 当前 linked AIE core kernel ABI、record header、c1r2 replay 和 bridge packet ABI，只把 `q4nx_chunk_accum_asm_zol` 继续改成更接近 MyLM `0x1f0..0x1850` 的 scheduled loop。这是当前最短路径。
...
- `qwen3-8b-decode-layer` 能用真实 MyLM Qwen3-8B-NPU2 权重跑通单层 full-layer frontier，并输出 2048-dword hidden payload。当前比较已经接到 `Qwen3LayerReference`，hidden_out 使用 `abs_tol=0.01, rel_tol=0.05`；token31 最新真机结果是 `28.479 ms`、`final_hidden_out max_abs=0.0078125`，current K/V、valid cache 和 capacity-unchanged cache 都是 0 mismatch。这已经是可运行的单层 decode frontier，但还不是多层 production 数值预算
...
| Main16 Q4NX kernel | 真 Q4NX transport/MAC 和真实模型权重 stream 已跑通；当前只保留 `main_projection_q4nx_fast.o` 一个 main16 role object，由 `main_projection_q4nx_fast.cc` scheduler 直接调用生成的 `main_projection_q4nx_asm.s` active hot body。旧 `q4nx_chunk_accum_fast` C++ wrapper 已从 active object 删除，实际计算在 `q4nx_chunk_accum_asm_zol`：静态形态为 `vmac.f=64`、`vextbcst.16=64`、`vextbcst.32=0`、`vunpack=64`、`vups.2x=32`、`vlda=2`、`vldb=66`、`vst=2`、`vconv.bf16.fp32=160`，并通过 generator/checker、`main16-q4nx-compute-perf`、QKV prefix 和 full decode 数值门。迁入后的 current full-decode `main_core_2_2.elf` `.text=7600`，没有旧 native lane symbol，final ELF 中保留 active asm symbol。最新优化把 activation group load 合并到每 32-dim 一次，并把 handoff nop 从 12 收到 6；`main16-q4nx-compute-perf=18.158 ms`，QKV prefix token31 `6.572 ms`，full decode token31 `28.479 ms`。row1/c1r1 compact tree 保持 MyLM-style record granularity：main16 17-dword record、row1 `17+16+16+16 -> 65`、c1r1 `65+64+64+64 -> 257` | 保持唯一 active asm hot body，继续压缩 dequant/conversion/call-state 开销，目标是让 active asm 的动态 MAC 密度接近 MyLM raw scheduled loop，而不是回退到 C++ 多变种 |
...
2. 只保留 `main_projection_q4nx_fast.cc` 作为 active main16 C++ implementation，active MLIR 统一调用 `q4nx_main16_layer_scheduler(..., phase_limit)`，不再维护 QKV-only、QKVO-only、scheduled-unroll 等并存变种。
...
6. 最新 root cause 已经从旧 phase-control 形态下移到 Q4NX microkernel 形态。active full-decode 现在统一调用一次 `q4nx_main16_layer_scheduler(..., phase_limit=7)`，QKV prefix/attention-O 也调用同一入口但传 `phase_limit=3/4`；旧的 `129` 次 Q4 helper call/`232` 次 lock control 是历史 direct-emit 问题。工具链 probe 仍然有价值：stock aiecc 的 child `opt/llc` flags 不可透传，`elf_file` 是 whole-core 替换入口；但下一步不是再调 no-unroll metadata/replay。exp99 的 aligned-ZOL exact lane 已经迁入唯一 active main16 linked-core implementation，并保持 full decode 数值正确；当前瓶颈是 active asm body 仍有大量 BF16/FP32 conversion、dequant `vmul/vadd` 和 call-state 保存/恢复。小步优化已证明有效：activation group load 复用和 6-nop handoff 把 `main16-q4nx-compute-perf` 从初始 asm `19.785 ms` 降到 `18.158 ms`，full decode token31 从 `29.489 ms` 降到 `28.479 ms`。后续优化必须在这个唯一 asm hot body 内提高动态 MAC 密度，而不是再维护 C++/asm 多套候选。
...
13. 汇编学习不能只停在 opcode 计数。exp123 已经把 MyLM group0
    `0x260..0x52a` 拆成逐 slot def/use 语义表：`192` slots、`28`
    `vmac.f`、`32` `vextbcst.16`、`8` `vups.4x`、`16`
    `vconv.bf16.fp32`、hot section 内 `vst=0`。关键结论是 group0 是
    pipeline fill，不是自闭合 32-MAC 小 kernel；它产生 `23` 个 data cell
    和 `4` 个 control cell 活到 group1，同时还从 entry 继承 `4` 个 data
    cell 和 `8` 个 control cell。后续写 exact 32-MAC/group 生成器时，必须
    先保住 fill-to-steady live state，再替换算术 body。
14. exp124 进一步拆了第一个 true steady-to-steady 组 group2。它在已有
    group0-1 live state 上运行，形态是 `189` slots、`33` `vmac.f`、`32`
    `vextbcst.16`、`8` `vups.4x`、`17` `vconv.bf16.fp32`、`vst=0`。boundary2
    和 boundary3 的 cell 名完全相同，都是 `27` 个 data cell 加 `12` 个
    control cell；相对 producer class 也稳定：`12` 个 entry、`3` 个
    persistent group0、`24` 个 previous-group。group2 做的事情是把其中
    `24` 个 producer 从 group1 替换成 group2，其中 `23` 个是 data cell、
    `1` 个是 control cell。这说明 steady 模板是 live-state transformer，
    不是独立函数体。
15. exp125 把当前 per-chunk helper-call 边界量化成 nocall scheduler
    contract。Q/K/V prefix 每个 main tile 需要 `192` 次
    `q4nx_chunk_accum_asm_zol` helper call，全层每 tile 是 `1472` 次；
    乘以 main16 后分别是 `3072` 和 `23552` 次。每次 helper call 有 `13`
    个 SAVE slots、`13` 个 RESTORE slots、2 次 accumulator load 和 2 次
    accumulator store；仅 SAVE/RESTORE 就是 QKV `4992` slots/tile、full
    layer `38272` slots/tile。这个边界确实值得消掉，但第一次把
    `q4nx_main16_qkv_scheduler_nocall` 直接接进 QKV prefix 后，静态检查
    能证明它不再 call helper，真机 `full-layer-qkv-prefix` 仍然 timeout；
    同一 topology 下旧 C++ scheduler 能过，所以根因在手写 scheduler 的
    lock/control/branch-delay 时序或 progress 位置，而不是 row1/c1r2/c1r3
    连线。active path 因此回到已知可跑的 `q4nx_main16_layer_scheduler`
    + `q4nx_chunk_accum_asm_zol`，QKV prefix 只通过 `phase_limit=3` 裁剪同一个入口，nocall 暂时只保留为实验结论。下一步如果
    继续做 scheduler，要先加 progress counter 定位停在 DMA0、DMA1、Q4 body
    还是 record emit；否则直接转向完整 Peano-shaped/whole-core main16
    scheduler，而不是把未验证的 nocall 半成品留在生产生成器里。
16. exp126 把“往 MyLM 方向”改成新的工程边界：不是再维护一个 QKV-only
    nocall 变种，而是生成一个 main16 role program。目标结构是 shared Q4NX
    body、Q/K/V body、O body、up/gate body、down body 和 dispatcher；first
    gate 仍然是 `full-layer-qkv-prefix token31`。同时明确第一版 raw/whole
    main16 replacement 必须保留 IRON compact header `10..15`，因为当前
    row1/c1r1/c1r3 routing 不是 MyLM 的 `0x1/0x4/0x8` header ABI。MyLM
    header 是 raw program 的逆向线索，不是可以直接塞进当前 IRON downstream
    的 ABI。
17. exp127 生成了第一个可汇编的 whole-main16 scaffold：entry、
    dispatcher、shared Q4 body、QKV/O/upgate/down phase body 都有稳定
    symbol。它已经把第一版 replacement 的 linked C ABI 固定下来：
    `p0..p5` 仍然是 weight/activation/record ping-pong buffer，`r0/r1/r2`
    是 group/row/num_rows；这样 MLIR topology、BD、lock 和 downstream
    compact routing 都不用同时换。manifest 还写入了 lock contract
    `48..53`、record header 公式，以及 full phase schedule：每个 main
    tile 动态是 `1472` 次 Q4 call、`76` 条 compact record。QKV body 不能用
    单一 IRON header，必须是 `Q packet10 x8 + K packet11 x2 + V packet12
    x2`；O 是 `packet13 x8`；up/gate 同属 `packet14` 但 phase 按 replay
    奇偶为 `4/5`；down 是 `packet15 x8`。最新生成的 record emitter 已经
    不是纯 placeholder：它会选中 ping/pong record buffer，按 IRON 公式写入
    header，把 `p6` 上的 32-lane FP32 accumulator 转成 16 个 BF16 payload
    dword，然后清空这个 FP32 accumulator。generator 还会检查 emitter 不引用
    caller 的 phase-loop state register，避免返回后破坏 record counter、lock
    token 或 tile metadata。phase body 现在不再持有 per-chunk loop，而是只配置
    header run 并调用 shared Q4 run body；全层静态只有 `6` 个 phase-to-Q4 run
    call，但 shared Q4 run body 动态执行 `1472` 个 chunk。shared Q4 run body
    持有 record loop、chunk loop、DMA lock protocol、ping/pong selection、
    exact chunk body 和 record emitter call。exact chunk body 从
    `qwen3-layer/tools/main16_q4nx_asm_lib.py` 生成，和 active
    `q4nx_chunk_accum_asm_zol` 使用同一份 asm source template，避免 exp127
    维护第二套 Q4NX hot body。当前嵌入 chunk body 已经是 no-save form，静态
    形态是 `64 vmac.f + 64 vextbcst.16 + 2 vst + 0 save/restore`。它的价值
    是把 shared Q4 run body、phase bodies、phase-limited dispatcher、record emitter 都放进
    同一个 generated whole-program artifact。exp127 的 future ABI 已经和 active `q4nx_main16_layer_scheduler` 对齐：`r3` 传 `phase_limit`，entry 保存到 `r31`，同一个 dispatcher 用 `3/4/6/7` 裁剪 QKV/QKVO/upgate/full。下一步不是再做 QKV-only
    scheduler，而是在这个 artifact 内把 current exact dequant body 换成
    MyLM-density scheduled body。这个 scaffold 还不是 active path。
18. active path 已同步成同一个 `q4nx_main16_layer_scheduler(..., phase_limit)`：
    QKV prefix 传 `3`，attention-O 传 `4`，full layer 传 `7`。`full-layer-qkv-prefix
    token31` 真机第一门已通过，说明 single-dispatcher phase limit 没破坏
    Q/K/V prefix 和 current K/V writeback；`qwen3-8b-decode-layer token31`
    也保持 PASS，当前 full-layer NPU time 是 `28.479 ms`，`final_hidden_out
    max_abs=0.0078125`、0 mismatch。
19. exp127 的 generated scaffold 补上了 nested-call `lr` contract。之前的
    scaffold 虽然能汇编，但 entry -> dispatcher -> phase body -> shared Q4
    body -> record emitter 这条调用链会反复覆盖 `lr`，不能作为可运行候选。
    现在 entry、dispatcher、phase bodies、shared Q4 body 都用合法的 64-byte
    stack frame 保存/恢复 `lr`，record emitter 被检查为 leaf。这个修正后，
    下一步接入 active path 时至少不会因为返回地址覆盖而假 timeout。
