...
WEIGHT_CHUNK_DWORDS = CHUNK_BF16 // 2
WEIGHT_CHUNKS_PER_TILE = sum(
    blocks * chunks for blocks, chunks in zip(PHASE_BLOCKS, PHASE_CHUNKS, strict=True)
)
MAIN16_WEIGHT_CHUNKS = WEIGHT_CHUNKS_PER_TILE * 16
...
def build_dataflow() -> LayerDataflow:
    nodes = (
        ...
        Node("shim_k", "c0r0", "current_k_writeback_and_split_kv_scan"),
        Node("shim_v", "c7r0", "current_v_writeback_and_split_kv_scan"),
        ...
        Node("row1_column_compact", "c2r1..c5r1", "per_column_compact_and_q4nx_weight_split"),
    ...
    edges = (
        ...
        Edge(
            ...
            "host",
            "row1_column_compact",
            "Q4NX weight chunks through c2..c5 row0 shims into row1 S2MM4/5",
            WEIGHT_CHUNK_DWORDS,
            None,
            MAIN16_WEIGHT_CHUNKS,
        ),
        Edge(
            "row1_weight_to_main",
            "row1_column_compact",
            "main16",
            "Q4NX weight chunks from row1 MM2S0..3 into main16 DMA1",
            WEIGHT_CHUNK_DWORDS,
            None,
            MAIN16_WEIGHT_CHUNKS,
        ),
        ...
        Edge("current_k_writeback", "postprocess", "shim_k", "current K", 512, 8, 1),
        Edge("current_v_writeback", "postprocess", "shim_v", "current V", 512, 9, 1),
        ...
        Edge("k_scan_left", "shim_k", "kv_left", "K groups 0..3 per rounded block", 4096, None, 1),
        Edge("v_scan_left", "shim_k", "kv_left", "V groups 0..3 per rounded block", 4096, None, 1),
        Edge("k_scan_right", "shim_v", "kv_right", "K groups 4..7 per rounded block", 4096, None, 1),
        Edge("v_scan_right", "shim_v", "kv_right", "V groups 4..7 per rounded block", 4096, None, 1),
        ...
        Edge("down_packet1", "hub", "activation_bridge", "FFN intermediate", 6144, 1, 1),
...
def validate_dataflow(graph: LayerDataflow | None = None) -> list[str]:
    ...
    expected_packets = {
        "current_k_writeback": 8,
        "current_v_writeback": 9,
        "attention_packet2": 2,
        "down_packet1": 1,
        "c1r2_qkv_replay": 0,
    ...
    for name in ("current_k_writeback", "current_v_writeback"):
        edge = edges.get(name)
        if edge is not None and edge.packet in (14, 15):
            errors.append(f"{name}: packet{edge.packet} conflicts with full-layer compact traffic")
    ...
    expected_sizes = {
        ...
        "global_compact": 257,
        "swiglu_to_hub": 256,
        "q4nx_weight_ingress": WEIGHT_CHUNK_DWORDS,
        "row1_weight_to_main": WEIGHT_CHUNK_DWORDS,
    }
