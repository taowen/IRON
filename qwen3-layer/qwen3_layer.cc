...
constexpr int ATTENTION_VECTOR_BF16 = NUM_Q_HEADS * HEAD_DIM;
constexpr int DOT_TAPS = 8;
...
static bfloat16 attention_cache[ATTENTION_VECTOR_BF16];
static uint8_t attention_cache_valid[ATTENTION_VECTOR_BF16];
...
static inline void clear_attention_cache() {
    for (int idx = 0; idx < ATTENTION_VECTOR_BF16; idx++) {
...
static inline float history_k_value(int32_t group, int32_t row, int32_t token, int32_t dim) {
    int32_t kv_head = group * 4 + row;
    int32_t raw = (kv_head * 13 + token * 7 + dim * 3) % 37;
...
static inline float history_v_value(int32_t group, int32_t row, int32_t token, int32_t dim) {
    int32_t kv_head = group * 4 + row;
    int32_t raw = (kv_head * 11 + token * 5 + dim * 2) % 31;
    return static_cast<float>(raw - 15) * 0.0125f;
}

static inline float global_q_value(int32_t head, int32_t dim) {
    int32_t block = head / 4;
    int32_t owner_group = head - block * 4;
    int32_t owner_row = dim / M_PER_TILE;
    int32_t lane = dim - owner_row * M_PER_TILE;
    int32_t raw = (
        block * 41 + owner_group * 17 + owner_row * 13 + lane * 7 + head * 3
    ) % 257;
    return static_cast<float>(raw - 128) * 0.001953125f;
}

static inline float global_current_k_value(int32_t kv_head, int32_t dim) {
    int32_t block = kv_head / 4;
    int32_t owner_group = kv_head - block * 4;
    int32_t owner_row = dim / M_PER_TILE;
    int32_t lane = dim - owner_row * M_PER_TILE;
    int32_t raw = (
        block * 43 + owner_group * 19 + owner_row * 11 + lane * 5 + kv_head * 7
    ) % 251;
    return static_cast<float>(raw - 125) * 0.00244140625f;
}

static inline float global_current_v_value(int32_t kv_head, int32_t dim) {
    int32_t block = kv_head / 4;
    int32_t owner_group = kv_head - block * 4;
    int32_t owner_row = dim / M_PER_TILE;
    int32_t lane = dim - owner_row * M_PER_TILE;
    ...
    return static_cast<float>(raw - 120) * 0.0029296875f;
}
...
static inline bfloat16 attention_output_value(
    ...
    int32_t head = global_idx / HEAD_DIM;
    int32_t dim = global_idx - head * HEAD_DIM;
    int32_t kv_head = head / 4;
    ...
    for (int tile = 0; tile < 2; tile++) {
        ...
        for (int token = 0; token < valid; token++) {
            ...
            float score = 0.0f;
            for (int tap = 0; tap < DOT_TAPS; tap++) {
                int tap_dim = (dim + tap * 17) & (HEAD_DIM - 1);
                float query = global_q_value(head, tap_dim);
                float key = global_token == CONTEXT_LEN - 1
                    ? global_current_k_value(kv_head, tap_dim)
                    : history_k_value(kv_head / 4, kv_head & 3, global_token, tap_dim);
                score += query * key;
            }
            score *= 0.3535533905932738f;
        ...
        for (int token = 0; token < valid; token++) {
            ...
            float value = global_token == CONTEXT_LEN - 1
                ? global_current_v_value(kv_head, dim)
                : history_v_value(kv_head / 4, kv_head & 3, global_token, dim);
            float score = 0.0f;
            for (int tap = 0; tap < DOT_TAPS; tap++) {
                int tap_dim = (dim + tap * 17) & (HEAD_DIM - 1);
                float query = global_q_value(head, tap_dim);
