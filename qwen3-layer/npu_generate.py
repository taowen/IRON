"""Generate runnable MLIR-AIE for the qwen3-layer NPU integration backend.

This backend is the executable hardware boundary for the current qwen3-layer:
it runs the full 608-patch Q/K/V/O/up/gate/down projection schedule and the
global QKV/O value contract on real NPU.  The separate generate.py file remains
the target qwen3-dataflow physical skeleton for the c1r1/c6r1/c1r2 layout.
"""
...
{chr(10).join(flows)}
    ...
    func.func private @clear_summary(memref<{M_PER_TILE}xbf16>, i32) attributes {{link_with = "{experiment_dir}/qwen3_layer.o"}}
    func.func private @emit_seed_sideband(memref<{RECORD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/qwen3_layer.o"}}
    func.func private @emit_next_block_sideband(memref<{RECORD_DWORDS}xi32>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, i32, i32, i32, i32, i32) attributes {{link_with = "{experiment_dir}/qwen3_layer.o"}}
    func.func private @accumulate_block_summary(memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/qwen3_layer.o"}}
    func.func private @edge_make_block_slice(memref<{RECORD_DWORDS}xi32>, memref<{ACT_SLICE_BF16}xbf16>, i32, i32, i32, i32, i32) attributes {{link_with = "{experiment_dir}/qwen3_layer.o"}}
    func.func private @q4nx_chunk_accum_slice(memref<{CHUNK_BF16}xbf16>, memref<{ACT_SLICE_BF16}xbf16>, i32) attributes {{link_with = "{experiment_dir}/qwen3_layer.o"}}
    func.func private @q4nx_flush_output(memref<{M_PER_TILE}xbf16>, i32) attributes {{link_with = "{experiment_dir}/qwen3_layer.o"}}
    func.func private @flush_schedule_output_with_header(memref<{OUT_RECORD_BF16}xbf16>, memref<{M_PER_TILE}xbf16>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/qwen3_layer.o"}}
