"""Patch currentkv decode schedule fields in generated NPU instructions."""
...
def _rtp_positions(words: np.ndarray) -> list[int]:
    ...
    for index in range(2, words.size - 3):
        if (
            words[index - 1] == 0
            and words[index + 1] == RTP_WRITE_TRAILER[0]
            and words[index + 2] == RTP_WRITE_TRAILER[1]
            and words[index + 3] == RTP_WRITE_TRAILER[2]
        ):
...
def _replace_positions(
    ...
    for position in positions:
        if words[position] != old_u32:
            raise ValueError(
                f"{label}: word {position} is 0x{int(words[position]):08x}, expected 0x{old_value & 0xFFFFFFFF:08x}"
            )
