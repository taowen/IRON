"""NPU runner for attention-kv16-o-bridge."""
...
import npu_build
from cases import attention_kv16_generate as generate
...
def build_kernel() -> tuple[Path, Path]:
    ...
    if errors:
        raise RuntimeError("\n".join(f"  ATTENTION KV16 STRUCTURE FAIL: {error}" for error in errors))
...
def check_only() -> bool:
    ...
    if errors:
        for error in errors:
            print(f"  ATTENTION KV16 STRUCTURE FAIL: {error}")
        return False
    print("  PASS: attention-kv16-o-bridge MLIR contains 16-token Shape-A/B handoff")
    return True
...
def run() -> bool:
    ...
    for line in route_summary():
        print(f"  {line}")
    print(
        "  route: host Q/KV -> c6r1/c0r1/c7r1 -> kv16 Shape-A/B -> "
    ...
    if errors:
        print(f"  FAIL: {len(errors)} attention-kv16 mismatches")
        for error in errors:
            print(f"    {error}")
        return False

    print("  PASS: 16-token attention producer reached main16 through packet2")
    return True
