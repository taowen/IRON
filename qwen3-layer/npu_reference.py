"""CPU reference for the runnable qwen3-layer NPU integration backend."""
...
NUM_Q_HEADS = 32
NUM_KV_HEADS = 8
ATTENTION_VECTOR_BF16 = NUM_Q_HEADS * HEAD_DIM
DOT_TAPS = 8
...
_GLOBAL_ATTENTION_VECTOR: np.ndarray | None = None
...
def edge_block_slice(
    ...
    if phase == O_PHASE:
        return global_attention_vector()[base : base + ACT_SLICE_BF16]

    global_idx = base + np.arange(ACT_SLICE_BF16, dtype=np.int32)
    ...
    mix = payload[lane] * np.float32(0.5 + 0.03125 * (phase + 1))
    mix = mix + payload[other] * np.float32(0.25 + 0.015625 * ((block & 7) + 1))
    mix = mix + (raw - 63).astype(np.float32) * np.float32(0.00390625)
    return mix.astype(bfloat16)
...
def approx_exp_vector(x: np.ndarray) -> np.ndarray:
    original = x.astype(np.float32, copy=False)
    clamped = np.minimum(original, np.float32(0.0))
    y = np.float32(1.0) + clamped * np.float32(1.0 / 64.0)
    for _ in range(6):
        y = (y * y).astype(np.float32)
    y = np.where(original <= np.float32(-16.0), np.float32(0.0), y)
    return y.astype(np.float32, copy=False)


def history_k_value(group: int, row: int, token: int, dim: int) -> np.float32:
    kv_head = group * 4 + row
    raw = (kv_head * 13 + token * 7 + dim * 3) % 37
    return np.float32((raw - 18) * 0.00875)


def history_v_value(group: int, row: int, token: int, dim: int) -> np.float32:
    kv_head = group * 4 + row
    raw = (kv_head * 11 + token * 5 + dim * 2) % 31
    return np.float32((raw - 15) * 0.0125)


def global_q_value(head: int, dim: int) -> np.float32:
    block = head // 4
    owner_group = head - block * 4
    owner_row = dim // M_PER_TILE
    lane = dim - owner_row * M_PER_TILE
    raw = (block * 41 + owner_group * 17 + owner_row * 13 + lane * 7 + head * 3) % 257
    return np.float32((raw - 128) * 0.001953125)


def global_current_k_value(kv_head: int, dim: int) -> np.float32:
    block = kv_head // 4
    owner_group = kv_head - block * 4
    owner_row = dim // M_PER_TILE
    lane = dim - owner_row * M_PER_TILE
    raw = (
        block * 43 + owner_group * 19 + owner_row * 11 + lane * 5 + kv_head * 7
    ) % 251
    return np.float32((raw - 125) * 0.00244140625)


def global_current_v_value(kv_head: int, dim: int) -> np.float32:
    block = kv_head // 4
    owner_group = kv_head - block * 4
    owner_row = dim // M_PER_TILE
    lane = dim - owner_row * M_PER_TILE
    raw = (
        block * 37 + owner_group * 23 + owner_row * 17 + lane * 3 + kv_head * 5
    ) % 241
    return np.float32((raw - 120) * 0.0029296875)


def _global_q_vector(heads: np.ndarray, dims: np.ndarray) -> np.ndarray:
    block = heads // 4
    owner_group = heads - block * 4
    owner_row = dims // M_PER_TILE
    lane = dims - owner_row * M_PER_TILE
    raw = (block * 41 + owner_group * 17 + owner_row * 13 + lane * 7 + heads * 3) % 257
    return (raw - 128).astype(np.float32) * np.float32(0.001953125)


def _global_current_k_vector(kv_heads: np.ndarray, dims: np.ndarray) -> np.ndarray:
    block = kv_heads // 4
    owner_group = kv_heads - block * 4
    owner_row = dims // M_PER_TILE
    lane = dims - owner_row * M_PER_TILE
    raw = (
        block * 43 + owner_group * 19 + owner_row * 11 + lane * 5 + kv_heads * 7
    ) % 251
    return (raw - 125).astype(np.float32) * np.float32(0.00244140625)


def _global_current_v_vector(kv_heads: np.ndarray, dims: np.ndarray) -> np.ndarray:
    block = kv_heads // 4
    owner_group = kv_heads - block * 4
    owner_row = dims // M_PER_TILE
    lane = dims - owner_row * M_PER_TILE
    raw = (
        block * 37 + owner_group * 23 + owner_row * 17 + lane * 3 + kv_heads * 5
    ) % 241
    return (raw - 120).astype(np.float32) * np.float32(0.0029296875)


def _history_k_vector(kv_heads: np.ndarray, token: int, dims: np.ndarray) -> np.ndarray:
    raw = (kv_heads * 13 + token * 7 + dims * 3) % 37
    return (raw - 18).astype(np.float32) * np.float32(0.00875)


def _history_v_vector(kv_heads: np.ndarray, token: int, dims: np.ndarray) -> np.ndarray:
    raw = (kv_heads * 11 + token * 5 + dims * 2) % 31
    return (raw - 15).astype(np.float32) * np.float32(0.0125)


def global_attention_vector() -> np.ndarray:
    global _GLOBAL_ATTENTION_VECTOR
    if _GLOBAL_ATTENTION_VECTOR is not None:
        return _GLOBAL_ATTENTION_VECTOR

    global_idx = np.arange(ATTENTION_VECTOR_BF16, dtype=np.int32)
    heads = global_idx // HEAD_DIM
    dims = global_idx - heads * HEAD_DIM
    kv_heads = heads // 4

    tap_dims = np.stack(
        [
            ((dims + tap * 17) & (HEAD_DIM - 1)).astype(np.int32)
            for tap in range(DOT_TAPS)
        ]
    )
    query_taps = np.stack(
        [_global_q_vector(heads, tap_dims[tap]) for tap in range(DOT_TAPS)]
    )

    running_max = np.full(ATTENTION_VECTOR_BF16, -np.inf, dtype=np.float32)
    running_sum = np.zeros(ATTENTION_VECTOR_BF16, dtype=np.float32)
    out = np.zeros(ATTENTION_VECTOR_BF16, dtype=np.float32)

    for tile in range(2):
        valid = TOKENS_PER_TILE if tile == 0 else CONTEXT_LEN - TOKENS_PER_TILE
        scores = np.empty((valid, ATTENTION_VECTOR_BF16), dtype=np.float32)
        for token in range(valid):
            global_token = tile * TOKENS_PER_TILE + token
            score = np.zeros(ATTENTION_VECTOR_BF16, dtype=np.float32)
            for tap in range(DOT_TAPS):
                if global_token == CONTEXT_LEN - 1:
                    key = _global_current_k_vector(kv_heads, tap_dims[tap])
                else:
                    key = _history_k_vector(kv_heads, global_token, tap_dims[tap])
                score = score + query_taps[tap] * key
            scores[token] = score * np.float32(0.3535533905932738)

        local_max = scores.max(axis=0)
        new_max = np.maximum(running_max, local_max)
        old_scale = np.where(
            running_sum == np.float32(0.0),
            np.float32(0.0),
            approx_exp_vector(running_max - new_max),
        )
        out = out * old_scale

        local_sum = np.zeros(ATTENTION_VECTOR_BF16, dtype=np.float32)
        for token in range(valid):
            global_token = tile * TOKENS_PER_TILE + token
            if global_token == CONTEXT_LEN - 1:
                value = _global_current_v_vector(kv_heads, dims)
            else:
                value = _history_v_vector(kv_heads, global_token, dims)
            weight = approx_exp_vector(scores[token] - new_max)
    ...
    result = np.where(
        running_sum == np.float32(0.0), np.float32(0.0), out / running_sum
    )
    _GLOBAL_ATTENTION_VECTOR = result.astype(bfloat16)
    return _GLOBAL_ATTENTION_VECTOR
...
class TileEdgeState:
    def __init__(self, group: int, row: int) -> None:
        ...
        self.attn_cache = np.zeros(ATTENTION_VECTOR_BF16, dtype=bfloat16)
        self.attn_valid = np.zeros(ATTENTION_VECTOR_BF16, dtype=bool)

    def remember(self, record: np.ndarray) -> None:
        tag = record_tag(record)
        if tag <= 0:
            self.attn_valid[:] = False
            return
    ...
    def attention_output_value(self, global_idx: int) -> bfloat16:
        ...
        head = global_idx // HEAD_DIM
        dim = global_idx - head * HEAD_DIM
        kv_head = head // 4
        ...
        for tile in range(2):
            ...
            for token in range(valid):
                ...
                for tap in range(DOT_TAPS):
                    tap_dim = (dim + tap * 17) & (HEAD_DIM - 1)
                    query = global_q_value(head, tap_dim)
                    if global_token == CONTEXT_LEN - 1:
                        key = global_current_k_value(kv_head, tap_dim)
                    else:
                        key = history_k_value(
                            kv_head // 4, kv_head & 3, global_token, tap_dim
                        )
                    score = np.float32(score + np.float32(query * key))
                score = np.float32(score * np.float32(0.3535533905932738))
            ...
            for token in range(valid):
                global_token = tile * TOKENS_PER_TILE + token
                if global_token == CONTEXT_LEN - 1:
                    value = global_current_v_value(kv_head, dim)
                else:
                    value = history_v_value(
                        kv_head // 4, kv_head & 3, global_token, dim
                    )
                score = np.float32(0.0)
                for tap in range(DOT_TAPS):
                    tap_dim = (dim + tap * 17) & (HEAD_DIM - 1)
