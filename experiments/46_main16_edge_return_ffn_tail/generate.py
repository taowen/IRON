"""Generate raw MLIR-AIE for exp46 main16 -> edge -> main16 O+FFN tail."""
...
def _main_tile(group: int, row: int) -> str:
    ...
    return f"""
    %{tile}_weight = aie.buffer(%{tile}) {{sym_name = "{tile}_weight"}} : memref<{TILE_WEIGHT_DWORDS}xi32>
...
{_lock_pair(tile, "output", 6)}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      func.call @main_o_phase(%{tile}_attention, %{tile}_weight, %{tile}_o_output, %group_i32, %row_i32)
        : (memref<{SHARD_DWORDS}xi32>, memref<{TILE_WEIGHT_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, i32, i32) -> ()
      func.call @main_ffn_tail(%{tile}_o_output, %{tile}_weight, %{tile}_gate, %{tile}_up, %{tile}_swiglu, %{tile}_output, %group_i32, %row_i32)
        : (memref<{SHARD_DWORDS}xi32>, memref<{TILE_WEIGHT_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, i32, i32) -> ()
    ...
    ^weight_in:
      ...
      aie.dma_bd(%{tile}_weight : memref<{TILE_WEIGHT_DWORDS}xi32>, 0, {TILE_WEIGHT_DWORDS}) {{bd_id = 0 : i32}}
...
{chr(10).join(flows)}
    ...
    func.func private @emit_main_record(memref<{RECORD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/edge_return_ffn_tail.o"}}
    func.func private @edge_make_attention_row(memref<{RECORD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/edge_return_ffn_tail.o"}}
    func.func private @main_o_phase(memref<{SHARD_DWORDS}xi32>, memref<{TILE_WEIGHT_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/edge_return_ffn_tail.o"}}
    func.func private @main_ffn_tail(memref<{SHARD_DWORDS}xi32>, memref<{TILE_WEIGHT_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/edge_return_ffn_tail.o"}}
