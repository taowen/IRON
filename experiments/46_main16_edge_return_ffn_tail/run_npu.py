#!/usr/bin/env python3
"""Run exp46 on real NPU and verify main16 -> edge -> main16 O+FFN tail."""
...
def compile_kernel() -> None:
    ...
    src = EXPERIMENT_DIR / "edge_return_ffn_tail.cc"
    obj = EXPERIMENT_DIR / "edge_return_ffn_tail.o"
    cmd = [
        str(clang),
        "-O2",
        "-std=c++20",
    ...
    print("  Compiling edge_return_ffn_tail.cc...")
    run_command(cmd)
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    ...
    required_markers = (
        ...
        f"memref<{TILE_WEIGHT_DWORDS}xi32>",
...
def run_on_npu() -> bool:
    print("=" * 72)
    print("Experiment 46: Main16 Edge Return FFN Tail")
    print("=" * 72)
    ...
    print(f"  tile weights: {TILE_WEIGHT_DWORDS} i32 = O + FFN")
    ...
    if mismatches.size == 0:
        print("  PASS: main16 -> edge -> main16 O+FFN tail verified.")
        return True
