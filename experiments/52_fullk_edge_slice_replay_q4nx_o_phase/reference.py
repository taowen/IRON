"""CPU reference for exp52 full-K edge shard slice replay feeding Q4NX O phase."""
...
def make_quantized_weights(seed: int = 52):
...
def expected_tile_record(packed: np.ndarray, group: int, row: int) -> np.ndarray:
    ...
    record[2:] = expected_tile_output(packed, group, row)
    return record
...
def make_packed_weights(seed: int = 52) -> np.ndarray:
...
if __name__ == "__main__":
    ...
    print(f"edge_shards={NUM_EDGE_SHARDS}, slices_per_shard={SLICES_PER_SHARD}")
    print(f"slice0[0:8]={attention_slice_from_record(0, 0, make_record(0, 0), 0)[:8].tolist()}")
    print(f"slice15[0:8]={attention_slice_from_record(0, 0, make_record(0, 0), 15)[:8].tolist()}")
