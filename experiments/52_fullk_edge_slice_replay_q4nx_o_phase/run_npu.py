#!/usr/bin/env python3
"""Run exp52 on real NPU and verify full-K edge-slice replay -> Q4NX O phase."""
...
from reference import attention_slice_from_record, expected_output, make_packed_weights, make_record
...
def compile_kernel() -> None:
    ...
    src = EXPERIMENT_DIR / "q4nx_edge_o_phase.cc"
    obj = EXPERIMENT_DIR / "q4nx_edge_o_phase.o"
    cmd = [
        str(clang),
        "-O2",
        "-std=c++20",
    ...
    print("  Compiling q4nx_edge_o_phase.cc...")
    run_command(cmd)
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    expected_counts = {
        ...
        "aiex.npu.writebd": len(MAIN_COLUMNS) * 2,
        "aiex.npu.address_patch": len(MAIN_COLUMNS) * 2,
        "aiex.npu.push_queue": len(MAIN_COLUMNS) * 2,
        "aiex.npu.sync": len(MAIN_COLUMNS),
        "func.call @emit_main_sideband": len(MAIN_COLUMNS) * ROWS_PER_COLUMN,
        "func.call @edge_make_attention_slice": len(MAIN_COLUMNS) * ROWS_PER_COLUMN * 2,
        "func.call @q4nx_chunk_accum_slice": len(MAIN_COLUMNS) * ROWS_PER_COLUMN * 2,
        "func.call @q4nx_flush_output_with_header": len(MAIN_COLUMNS) * ROWS_PER_COLUMN,
    }
    errors: list[str] = []
    ...
    forbidden_markers = (
        f"memref<{O_INPUT_DIM}xbf16>",
        f"memref<{EDGE_SHARD_BF16}xbf16>",
    ...
    for marker in forbidden_markers:
        if marker in mlir_text:
            errors.append(f"Forbidden old/full-shard marker found: {marker}")
...
def run_on_npu() -> bool:
    print("=" * 78)
    print("Experiment 52: Full-K Edge Slice Replay To Q4NX O Phase")
    print("=" * 78)
    print(f"  main columns: {MAIN_COLUMNS}, edge columns: {EDGE_COLUMNS}")
    print(f"  rows per column: {ROWS_PER_COLUMN}")
    print(f"  sideband: {RECORD_DWORDS} i32")
    print(
        f"  edge replay: {NUM_EDGE_SHARDS} shards x {SLICES_PER_SHARD} slices "
        f"x {ACT_SLICE_BF16} bf16 = {O_INPUT_DIM // 2} dwords"
    )
    print(f"  per tile O phase: M={M_PER_TILE}, K={O_INPUT_DIM}, K_CHUNK={K_CHUNK}, chunks={NUM_CHUNKS}")
    print(f"  Q4NX chunk: {CHUNK_BF16 * 2} bytes, fat chunk: {FAT_CHUNK_BF16 * 2} bytes")
    print(f"  column O stream: {COLUMN_WEIGHT_BF16 * 2} bytes")
    ...
    print(f"  group0 record0: {record.tolist()}")
    print(f"  group0 slice0[0:8]: {attention_slice_from_record(0, 0, record, 0)[:8].tolist()}")
    print(f"  group0 slice15[0:8]: {attention_slice_from_record(0, 0, record, 15)[:8].tolist()}")
    ...
    if mismatch.size == 0:
        print(
            "  PASS: four 512-dword edge shards replay as 16 x 128-dword slices "
            "into full-K Q4NX O phase; debug packet gather was reordered by record header."
        )
        return True
