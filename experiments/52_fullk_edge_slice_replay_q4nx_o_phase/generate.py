"""Generate raw MLIR-AIE for exp52 full-K edge-slice replay -> Q4NX O phase."""
...
M_PER_TILE = 32
O_INPUT_DIM = 4096
EDGE_SHARD_BF16 = 1024
ACT_SLICE_BF16 = 256
K_CHUNK = 256
NUM_EDGE_SHARDS = O_INPUT_DIM // EDGE_SHARD_BF16
SLICES_PER_SHARD = EDGE_SHARD_BF16 // ACT_SLICE_BF16
...
{_main_chunk_body(tile)}
      ...
      func.call @q4nx_flush_output_with_header(%{tile}_output, %group_i32, %row_i32, %m_i32)
...
{_lock_pair(tile, "act", 2, init_empty=2)}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      %c1_i32 = arith.constant 1 : i32
      %c2_i32 = arith.constant 2 : i32
      %num_shards = arith.constant {NUM_EDGE_SHARDS} : index
      %slices_per_shard = arith.constant {SLICES_PER_SHARD} : index
      %slices_per_shard_i32 = arith.constant {SLICES_PER_SHARD} : i32
      %group_i32 = arith.constant {group} : i32
      %row_i32 = arith.constant {row} : i32

      aie.use_lock(%{tile}_record_full, AcquireGreaterEqual, 1)
      scf.for %shard = %c0 to %num_shards step %c1 {{
        %shard_i32 = arith.index_cast %shard : index to i32
        %chunk_base = arith.muli %shard_i32, %slices_per_shard_i32 : i32
        scf.for %slice = %c0 to %slices_per_shard step %c1 {{
          %slice_i32 = arith.index_cast %slice : index to i32
          %chunk_i32 = arith.addi %chunk_base, %slice_i32 : i32
          %rem = arith.remsi %chunk_i32, %c2_i32 : i32
          %is_pong = arith.cmpi eq, %rem, %c1_i32 : i32
          aie.use_lock(%{tile}_act_empty, AcquireGreaterEqual, 1)
          scf.if %is_pong {{
...
{chr(10).join(flows)}
    ...
    func.func private @q4nx_flush_output_with_header(memref<{OUT_RECORD_BF16}xbf16>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/q4nx_edge_o_phase.o"}}
