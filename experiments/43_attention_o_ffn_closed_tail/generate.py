"""Generate raw MLIR-AIE for exp43 attention + O projection + FFN closed tail."""
...
def _edge_attention() -> str:
    ...
    %edge_attention_lo = aie.buffer(%edge_attn) {{sym_name = "edge_attention_lo"}} : memref<{HALF_DWORDS}xi32>
    %edge_attention_hi = aie.buffer(%edge_attn) {{sym_name = "edge_attention_hi"}} : memref<{HALF_DWORDS}xi32>
...
{_lock_pair("edge_attn", "attention_hi", 6)}
    ...
    %edge_attn_core = aie.core(%edge_attn) {{
      ...
      func.call @edge_make_attention(%edge_current, %edge_history, %edge_attention_lo, %edge_attention_hi)
        : (memref<{CURRENT_DWORDS}xi32>, memref<{HISTORY_DWORDS}xi32>, memref<{HALF_DWORDS}xi32>, memref<{HALF_DWORDS}xi32>) -> ()
    ...
    ^attention_lo:
      ...
      aie.dma_bd(%edge_attention_lo : memref<{HALF_DWORDS}xi32>, 0, {HALF_DWORDS}) {{bd_id = 2 : i32, next_bd_id = 3 : i32}}
    ...
    ^attention_hi:
      ...
      aie.dma_bd(%edge_attention_hi : memref<{HALF_DWORDS}xi32>, 0, {HALF_DWORDS}) {{bd_id = 3 : i32, next_bd_id = 2 : i32}}
...
def _runtime_sequence() -> str:
    return "\n".join(
        [
            ...
            _npu_writebd(2, 2, OUTPUT_DWORDS, 0),
            _npu_address_patch(2, 2, 4, 0),
            _npu_push_queue(2, "S2MM", 0, 2, issue_token=True),
            ...
            _npu_writebd(2, 0, FFN_WEIGHT_DWORDS, 0),
            _npu_address_patch(2, 0, 3, 0),
...
def generate_mlir() -> str:
  ...
  aie.device(npu2) {{
    ...
    aie.flow(%shim2, DMA : 0, %ffn_tail, DMA : 1)
    aie.flow(%ffn_tail, DMA : 0, %shim2, DMA : 0)

    func.func private @edge_make_attention(memref<{CURRENT_DWORDS}xi32>, memref<{HISTORY_DWORDS}xi32>, memref<{HALF_DWORDS}xi32>, memref<{HALF_DWORDS}xi32>) attributes {{link_with = "{experiment_dir}/closed_tail.o"}}
    func.func private @o_project_attention(memref<{HALF_DWORDS}xi32>, memref<{HALF_DWORDS}xi32>, memref<{O_WEIGHT_DWORDS}xi32>, memref<{HALF_DWORDS}xi32>, memref<{HALF_DWORDS}xi32>) attributes {{link_with = "{experiment_dir}/closed_tail.o"}}
    func.func private @ffn_tail(memref<{HALF_DWORDS}xi32>, memref<{HALF_DWORDS}xi32>, memref<{FFN_WEIGHT_DWORDS}xi32>, memref<{OUTPUT_DWORDS}xi32>, memref<{OUTPUT_DWORDS}xi32>, memref<{OUTPUT_DWORDS}xi32>, memref<{OUTPUT_DWORDS}xi32>) attributes {{link_with = "{experiment_dir}/closed_tail.o"}}
