"""Generate raw MLIR-AIE for exp48 main16 full-K Q4NX phase replay."""
...
def _main_core_phase(tile: str, phase_name: str, phase_buffer: str) -> str:
    return f"""
      // {phase_name} full-K Q4NX phase
      scf.for %chunk = %c0 to %num_chunks step %c1 {{
        ...
        scf.if %is_pong {{
          func.call @q4nx_chunk_accum_offset(%{tile}_wt_pong, %{tile}_act, %act_offset, %m_i32)
            : (memref<{CHUNK_BF16}xbf16>, memref<{ACT_BF16}xbf16>, i32, i32) -> ()
        }} else {{
          func.call @q4nx_chunk_accum_offset(%{tile}_wt_ping, %{tile}_act, %act_offset, %m_i32)
            : (memref<{CHUNK_BF16}xbf16>, memref<{ACT_BF16}xbf16>, i32, i32) -> ()
        }}
        aie.use_lock(%{tile}_wt_empty, Release, 1)
      }}
      func.call @q4nx_flush_output(%{tile}_{phase_buffer}, %m_i32)
...
def _main_tile(group: int, row: int) -> str:
    ...
    return f"""
    %{tile}_act = aie.buffer(%{tile}) {{sym_name = "{tile}_act"}} : memref<{ACT_BF16}xbf16>
    ...
    %{tile}_o = aie.buffer(%{tile}) {{sym_name = "{tile}_o"}} : memref<{OUT_BF16}xbf16>
    %{tile}_gate = aie.buffer(%{tile}) {{sym_name = "{tile}_gate"}} : memref<{OUT_BF16}xbf16>
    %{tile}_up = aie.buffer(%{tile}) {{sym_name = "{tile}_up"}} : memref<{OUT_BF16}xbf16>
    %{tile}_down = aie.buffer(%{tile}) {{sym_name = "{tile}_down"}} : memref<{OUT_BF16}xbf16>
    ...
    %{tile}_out_empty = aie.lock(%{tile}, 2) {{init = 1 : i32, sym_name = "{tile}_out_empty"}}
    %{tile}_out_full = aie.lock(%{tile}, 3) {{init = 0 : i32, sym_name = "{tile}_out_full"}}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      func.call @init_activation(%{tile}_act, %group_i32, %row_i32)
        : (memref<{ACT_BF16}xbf16>, i32, i32) -> ()
{_main_core_phase(tile, "O", "o")}
{_main_core_phase(tile, "GATE", "gate")}
{_main_core_phase(tile, "UP", "up")}
{_main_core_phase(tile, "DOWN", "down")}
      aie.use_lock(%{tile}_out_empty, AcquireGreaterEqual, 1)
      func.call @combine_q4nx_phases(%{tile}_o, %{tile}_gate, %{tile}_up, %{tile}_down, %{tile}_output, %group_i32, %row_i32)
        : (memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, i32, i32) -> ()
      aie.use_lock(%{tile}_out_full, Release, 1)
      aie.end
    }}

    %{tile}_mem = aie.mem(%{tile}) {{
      %0 = aie.dma_start(S2MM, 0, ^wt_ping, ^out_start)
    ^wt_ping:
      aie.use_lock(%{tile}_wt_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{tile}_wt_ping : memref<{CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 0 : i32, next_bd_id = 1 : i32}}
      aie.use_lock(%{tile}_wt_full, Release, 1)
      aie.next_bd ^wt_pong
    ^wt_pong:
      aie.use_lock(%{tile}_wt_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{tile}_wt_pong : memref<{CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 1 : i32, next_bd_id = 0 : i32}}
      aie.use_lock(%{tile}_wt_full, Release, 1)
      aie.next_bd ^wt_ping
    ...
    ^out_bd:
      ...
      aie.dma_bd(%{tile}_output : memref<{OUT_BF16}xbf16>, 0, {OUT_BF16}) {{bd_id = 2 : i32, packet = #aie.packet_info<pkt_type = 0, pkt_id = {_output_packet(group, row)}>}}
...
def _runtime_sequence() -> str:
    ...
    for group, column in enumerate(MAIN_COLUMNS):
        ...
        for phase in range(NUM_PHASES):
            bd_id = phase
            weight_offset = (group * COLUMN_WEIGHT_BF16 + phase * PHASE_WEIGHT_BF16) * 2
            lines += [
                _npu_writebd(column, bd_id, PHASE_WEIGHT_BF16 // 2, weight_offset),
                _npu_address_patch(column, bd_id, 0, weight_offset),
                _npu_push_queue(column, "MM2S", 0, bd_id, issue_token=(phase == NUM_PHASES - 1)),
            ]
...
def generate_mlir() -> str:
    ...
    for group, column in enumerate(MAIN_COLUMNS):
        ...
        for row in range(ROWS_PER_COLUMN):
            tile_defs.append(f"    %m{group}_{row} = aie.tile({column}, {row + 2})")
...
{chr(10).join(flows)}
    ...
    func.func private @init_activation(memref<{ACT_BF16}xbf16>, i32, i32) attributes {{link_with = "{experiment_dir}/q4nx_phase_replay.o"}}
    func.func private @q4nx_chunk_accum_offset(memref<{CHUNK_BF16}xbf16>, memref<{ACT_BF16}xbf16>, i32, i32) attributes {{link_with = "{experiment_dir}/q4nx_phase_replay.o"}}
    func.func private @q4nx_flush_output(memref<{OUT_BF16}xbf16>, i32) attributes {{link_with = "{experiment_dir}/q4nx_phase_replay.o"}}
    func.func private @combine_q4nx_phases(memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, i32, i32) attributes {{link_with = "{experiment_dir}/q4nx_phase_replay.o"}}
