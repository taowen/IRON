#!/usr/bin/env python3
"""
Experiment 11: Multi-Chunk Weight-Streaming Q4NX GEMV on NPU.

Proves:
1. Multi-chunk weight streaming (K=1024 as 4x256 chunks, double-buffered)
2. Lock-driven DMA/compute overlap
3. FP32 accumulation across chunks
4. Numerical equivalence with CPU reference
"""
...
from design import chunked_q4nx_pipeline, M, K, K_CHUNK, NUM_CHUNKS, GROUP_SIZE
from reference import pack_all_chunks, q4nx_matvec_reference
...
def compile_kernel(build_dir):
    ...
    src = EXPERIMENT_DIR / "q4nx_matvec_accum.cc"
    obj = EXPERIMENT_DIR / "q4nx_matvec_accum.o"

    cmd = [
        str(clang), "-O2", "-std=c++20",
        "--target=aie2p-none-unknown-elf",
        f"-DQ4_M={M}", f"-DQ4_K_CHUNK={K_CHUNK}", f"-DGROUP_SIZE={GROUP_SIZE}",
    ...
    print(f"  Compiling q4nx_matvec_accum.cc...")
...
def run_on_npu():
    ...
    print(f"Configuration: M={M}, K={K}, K_CHUNK={K_CHUNK}, NUM_CHUNKS={NUM_CHUNKS}")
    print(f"Q4NX chunk: {chunk_bytes} bytes, total weight: {chunk_bytes * NUM_CHUNKS} bytes")
    print()
    ...
    # Generate MLIR
    mlir_module = chunked_q4nx_pipeline(dev=dev)
    ...
    # Pack all chunks into single weight buffer
    packed = pack_all_chunks(scales, zeros, int4_data)
    assert packed.shape[0] == chunk_bytes * NUM_CHUNKS

    # CPU reference
    ref_output = q4nx_matvec_reference(packed, activation)
    ...
    print("=" * 60)
    if num_errors == 0:
        print("SUCCESS: Multi-chunk Q4NX GEMV produces CORRECT output!")
        print(f"  K={K} processed as {NUM_CHUNKS} x {K_CHUNK} chunks")
        print(f"  Double-buffered weight streaming with lock handshake")
        print(f"  FP32 accumulation across {NUM_CHUNKS} chunks")
        print(f"  Latency: {npu_time_us:.1f} us for M={M} K={K} Q4NX matmul")
    else:
...
if __name__ == "__main__":
    print("=" * 70)
    print("Experiment 11: Multi-Chunk Weight-Streaming Q4NX GEMV")
    print("=" * 70)
    print()
