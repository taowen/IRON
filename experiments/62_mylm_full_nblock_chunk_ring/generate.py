"""Generate raw MLIR-AIE for exp62 full N-block chunk-ring projection."""

import os
from pathlib import Path

VARIANT = os.environ.get("EXP62_VARIANT", "full")
if VARIANT in ("onecol", "split_onecol"):
    MAIN_COLUMNS = (2,)
    EDGE_COLUMNS = (0,)
elif VARIANT in ("full", "split_full"):
    MAIN_COLUMNS = (2, 3, 4, 5)
    EDGE_COLUMNS = (0, 1, 6, 7)
else:
    raise ValueError(f"unsupported EXP62_VARIANT={VARIANT!r}")
SPLIT_INPUT = VARIANT.startswith("split_")
...
# The same-channel variants intentionally allocate 32 input phases on S2MM ch0.
# Phases 16..31 cross into odd-channel BD slots; that reproduces the timeout
# root cause. Split variants keep the same logical phase shape but put patch1 on
# channel 1 with legal odd-bank BDs.
INPUT_BD_IDS = tuple(
    list(range(0, 2))
    + [6]
    + list(range(11, 24))
...
def _input_chunk_bd(group: int, phase: int, bd_id: int, next_bd_id: int) -> str:
    prefix = f"mt{group}"
    patch = phase // K_CHUNKS
    chunk = phase % K_CHUNKS
    slot = "pong" if chunk % 2 else "ping"
    next_phase = (phase + 1) % (PATCHES_PER_COLUMN * K_CHUNKS)
    next_label = f"^patch{next_phase // K_CHUNKS}_chunk{next_phase % K_CHUNKS}"
    return f"""    ^patch{patch}_chunk{chunk}:
      aie.use_lock({_slot_lock(prefix, patch, slot, "empty")}, AcquireGreaterEqual, 2)
      aie.dma_bd(%{prefix}_patch{patch}_{slot} : memref<{2 * CHUNK_BF16}xbf16>, 0, {2 * CHUNK_BF16}) {{bd_id = {bd_id} : i32, next_bd_id = {next_bd_id} : i32}}
      aie.use_lock({_slot_lock(prefix, patch, slot, "full")}, Release, 2)
      aie.next_bd {next_label}"""


def _input_chunk_bds(group: int) -> str:
    lines = []
    total_phases = PATCHES_PER_COLUMN * K_CHUNKS
    for phase in range(total_phases):
        lines.append(
            _input_chunk_bd(
                ...
                phase,
                INPUT_BD_IDS[phase],
                INPUT_BD_IDS[(phase + 1) % total_phases],
            )
        )
    return "\n".join(lines)
...
def _input_dma_start(group: int) -> str:
    if not SPLIT_INPUT:
        return f"""      %0 = aie.dma_start(S2MM, 0, ^patch0_chunk0, ^row0_start)
{_input_chunk_bds(group)}"""
    return "\n".join(
        (
...
def _runtime_sequence() -> str:
    ...
    output_channel = 0 if SPLIT_INPUT else 1
    ...
    for group, column in enumerate(MAIN_COLUMNS):
        ...
        if SPLIT_INPUT:
            lines += [
                _npu_writebd(column, 0, PATCH_BF16 // 2, column_base),
                ...
                _npu_push_queue(column, "MM2S", 1, 1, issue_token=True),
                _npu_sync(column, 0, direction=1),
                _npu_sync(column, 1, direction=1),
            ]
        else:
            lines += [
                _npu_writebd(
...
def generate_mlir() -> str:
    ...
    for group, main_col in enumerate(MAIN_COLUMNS):
        ...
        if SPLIT_INPUT:
            flows.append(f"    aie.flow(%shim{group}, DMA : 1, %mt{group}, DMA : 1)")
