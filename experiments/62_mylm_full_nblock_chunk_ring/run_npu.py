#!/usr/bin/env python3
"""Run exp62 on real NPU and verify full N-block chunk-ring projection."""
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    ...
    expected_flows_per_column = ROWS_PER_COLUMN * 2 + (3 if SPLIT_INPUT else 2)
    expected_host_queues = 3 if SPLIT_INPUT else 2
    expected_host_syncs = 3 if SPLIT_INPUT else 2
    expected_counts = {
        "aie.flow(": len(MAIN_COLUMNS) * expected_flows_per_column,
        "aie.packet_flow": expected_tiles,
        ...
        "aiex.npu.push_queue": len(MAIN_COLUMNS) * expected_host_queues,
        "aiex.npu.sync": len(MAIN_COLUMNS) * expected_host_syncs,
        "use_next_bd = 1": 0 if SPLIT_INPUT else len(MAIN_COLUMNS),
        "func.call @edge_make_activation_slice": expected_tiles * 2,
    ...
    if SPLIT_INPUT:
        ...
        required_markers += (
            "aie.flow(%mt0, DMA : 5, %shim0, DMA : 1)",
            "aie.dma_start(S2MM, 0, ^patch0_chunk0, ^row0_start)",
...
def run_on_npu() -> bool:
    print("=" * 78)
    print("Experiment 62: MyLM Full N-Block Chunk Ring")
    print("=" * 78)
    ...
    print("  Preparing exact patch-layout Q4NX weights and CPU reference...")
    ...
    if mismatch.size == 0:
        print(f"  PASS: {VARIANT} streams exact MyLM patches through row1 chunk rings.")
        return True
