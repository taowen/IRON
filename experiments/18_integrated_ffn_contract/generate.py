...
This experiment keeps the smallest topology that exercises the hard FFN
dataflow contract:
- one column, four compute cores
- gate/up/down projections reuse the same worker and weight path over time
- gate/up outputs stay core-local until SwiGLU
- SwiGLU outputs gather in the memtile and broadcast back for down projection
- each ping/pong slot has its own empty/full lock pair
- runtime never rewrites a queued shim BD descriptor before synchronization
"""
...
PUSHES_PER_COL = NUM_PHASES_PROJ * NUM_CHUNKS + NUM_CHUNKS_DOWN  # 5
PER_COL_WT_BF16 = PUSHES_PER_COL * FAT_CHUNK_BF16  # 5 x 10240 = 51200
...
def _memtile_dma(col):
    """Memtile DMA for a 4-core column.

    S2MM ch0 (even, BD 0): activation from shim
    S2MM ch1 (odd, BD 24-25): weights from shim, ping-pong
...
def generate_mlir() -> str:
    ...
    # Runtime sequence.  Weight pushes use distinct shim BD descriptors because
    # shim queues consume descriptors asynchronously.
    rt_lines = []
    ...
    # Phase 2: arm output receiver before weights.
    for col in range(NUM_COLS):
