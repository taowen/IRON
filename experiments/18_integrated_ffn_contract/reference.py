"""CPU reference for experiment 18's one-column fused FFN contract."""
...
def pack_all_weights(all_scales, all_zeros, all_int4, down_scales, down_zeros, down_int4):
    ...
    Layout per column: [2 gate_fat | 2 up_fat | 1 down_fat_padded]
    ...
    for col in range(NUM_COLS):
        ...
        for row in range(ROWS_PER_COL):
            ...
            sigmoid_gate = np.where(
                gate_f32 >= 0,
                1.0 / (1.0 + np.exp(-gate_f32)),
                np.exp(gate_f32) / (1.0 + np.exp(gate_f32)),
            )
...
if __name__ == "__main__":
    ...
    print(f"Config: {NUM_COLS} col × {ROWS_PER_COL} rows, K={K_HIDDEN}, complete FFN")
    ...
    print(f"Output[96:100] (col0_r3): {output[96:100]}")
