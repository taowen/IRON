#!/usr/bin/env python3
"""Phase-level diagnostics for experiment 18.
...
def _q4_loop(prefix: str, phase_offset: int, out_name: str) -> str:
    offset_name = f"{out_name}_phase_offset_i32"
    return f"""
      %{offset_name} = arith.constant {phase_offset} : i32
      scf.for %chunk = %c0 to %num_chunks step %c1 {{
        ...
        %shifted = arith.addi %chunk_i32, %{offset_name} : i32
...
def _probe_core_program(mode: str, col: int, row: int, prefix: str) -> str:
    del col, row
    core_mode = mode.removesuffix("-extra")
