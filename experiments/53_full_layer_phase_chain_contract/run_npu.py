#!/usr/bin/env python3
"""Run exp53 on real NPU and verify the full fused-layer phase chain."""
...
def compile_kernel() -> None:
    ...
    src = EXPERIMENT_DIR / "phase_chain.cc"
    obj = EXPERIMENT_DIR / "phase_chain.o"
    cmd = [
        str(clang),
        "-O2",
        "-std=c++20",
    ...
    print("  Compiling phase_chain.cc...")
    run_command(cmd)
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    expected_counts = {
        ...
        "func.call @main_emit_phase_record": len(MAIN_COLUMNS) * ROWS_PER_COLUMN,
        "func.call @edge_make_phase_slice": len(MAIN_COLUMNS) * ROWS_PER_COLUMN * 2,
        ...
        "func.call @flush_layer_output_with_header": len(MAIN_COLUMNS) * ROWS_PER_COLUMN,
    }
    errors: list[str] = []
...
def run_on_npu() -> bool:
    print("=" * 78)
    print("Experiment 53: Full Layer Phase Chain Contract")
    print("=" * 78)
    ...
    print(f"  per phase: M={M_PER_TILE}, K={HIDDEN_DIM}, chunks={NUM_CHUNKS}, slice={ACT_SLICE_BF16} bf16")
    print(f"  Q4NX chunk: {CHUNK_BF16 * 2} bytes, fat chunk: {FAT_CHUNK_BF16 * 2} bytes")
    print(f"  per-column phase stream: {PHASE_WEIGHT_BF16 * 2} bytes")
    ...
    if mismatch.size == 0:
        print(
            "  PASS: Q/K/V -> edge attention replay -> O -> gate/up -> SwiGLU -> down "
            "ran as one fused phase chain on the same main16/edge16 fabric."
        )
        return True
