"""Generate raw MLIR-AIE for exp53 full fused-layer phase-chain contract."""
...
def _main_tile(group: int, row: int) -> str:
    ...
    %{tile}_o = aie.buffer(%{tile}) {{sym_name = "{tile}_o"}} : memref<{M_PER_TILE}xbf16>
    %{tile}_gate = aie.buffer(%{tile}) {{sym_name = "{tile}_gate"}} : memref<{M_PER_TILE}xbf16>
    %{tile}_up = aie.buffer(%{tile}) {{sym_name = "{tile}_up"}} : memref<{M_PER_TILE}xbf16>
    %{tile}_swiglu = aie.buffer(%{tile}) {{sym_name = "{tile}_swiglu"}} : memref<{M_PER_TILE}xbf16>
...
{_lock_pair(tile, "output", 6)}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      %num_phases = arith.constant {NUM_PHASES} : index
      %last_phase_i32 = arith.constant {NUM_PHASES - 1} : i32
      ...
      scf.for %phase = %c0 to %num_phases step %c1 {{
        %phase_i32 = arith.index_cast %phase : index to i32
        ...
        %is_last_phase = arith.cmpi eq, %phase_i32, %last_phase_i32 : i32
        scf.if %is_last_phase {{
          aie.use_lock(%{tile}_output_empty, AcquireGreaterEqual, 1)
          func.call @flush_layer_output_with_header(%{tile}_output, %{tile}_o, %{tile}_gate, %{tile}_up, %{tile}_swiglu, %{tile}_phase, %group_i32, %row_i32, %m_i32)
            : (memref<{OUT_RECORD_BF16}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, i32, i32, i32) -> ()
          aie.use_lock(%{tile}_output_full, Release, 1)
        }} else {{
          aie.use_lock(%{tile}_record_empty, AcquireGreaterEqual, 1)
          func.call @main_emit_phase_record(%{tile}_record, %{tile}_phase, %{tile}_o, %{tile}_gate, %{tile}_up, %{tile}_swiglu, %phase_i32, %group_i32, %row_i32, %m_i32)
            : (memref<{RECORD_DWORDS}xi32>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, i32, i32, i32, i32) -> ()
          aie.use_lock(%{tile}_record_full, Release, 1)
        }}
      }}
      aie.end
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      scf.for %phase = %c0 to %num_phases step %c1 {{
        ...
        scf.for %chunk = %c0 to %num_chunks step %c1 {{
          ...
          aie.use_lock(%{tile}_act_empty, AcquireGreaterEqual, 1)
          scf.if %is_pong {{
            func.call @edge_make_phase_slice(%{tile}_record, %{tile}_act_pong, %phase_i32, %chunk_i32, %group_i32, %row_i32)
              : (memref<{RECORD_DWORDS}xi32>, memref<{ACT_SLICE_BF16}xbf16>, i32, i32, i32, i32) -> ()
          }} else {{
            func.call @edge_make_phase_slice(%{tile}_record, %{tile}_act_ping, %phase_i32, %chunk_i32, %group_i32, %row_i32)
              : (memref<{RECORD_DWORDS}xi32>, memref<{ACT_SLICE_BF16}xbf16>, i32, i32, i32, i32) -> ()
          }}
          aie.use_lock(%{tile}_act_full, Release, 1)
        }}
...
def _memtile_column(group: int) -> str:
    ...
    for bd_index, row in enumerate(range(ROWS_PER_COLUMN)):
      ...
      aie.use_lock(%{tile}_output_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{tile}_output : memref<{COLUMN_OUTPUT_BF16}xbf16>, {bd_index * OUT_RECORD_BF16}, {OUT_RECORD_BF16}) {{bd_id = {7 + bd_index} : i32, next_bd_id = {7 + next_bd_index} : i32}}
      aie.use_lock(%{tile}_output_full, Release, 1)
    ...
    %{tile}_wt_ping_empty = aie.lock(%{tile}, 0) {{init = {ROWS_PER_COLUMN} : i32, sym_name = "{tile}_wt_ping_empty"}}
    %{tile}_wt_ping_full = aie.lock(%{tile}, 1) {{init = 0 : i32, sym_name = "{tile}_wt_ping_full"}}
    ...
    %{tile}_wt_pong_empty = aie.lock(%{tile}, 4) {{init = {ROWS_PER_COLUMN} : i32, sym_name = "{tile}_wt_pong_empty"}}
    %{tile}_wt_pong_full = aie.lock(%{tile}, 5) {{init = 0 : i32, sym_name = "{tile}_wt_pong_full"}}

    %{tile}_dma = aie.memtile_dma(%{tile}) {{
      %0 = aie.dma_start(S2MM, 0, ^wt_in_ping, ^row0_start)
    ^wt_in_ping:
      aie.use_lock(%{tile}_wt_ping_empty, AcquireGreaterEqual, {ROWS_PER_COLUMN})
      aie.dma_bd(%{tile}_wt_ping : memref<{FAT_CHUNK_BF16}xbf16>, 0, {FAT_CHUNK_BF16}) {{bd_id = 0 : i32, next_bd_id = 1 : i32}}
      aie.use_lock(%{tile}_wt_ping_full, Release, {ROWS_PER_COLUMN})
      aie.next_bd ^wt_in_pong
    ^wt_in_pong:
      aie.use_lock(%{tile}_wt_pong_empty, AcquireGreaterEqual, {ROWS_PER_COLUMN})
      aie.dma_bd(%{tile}_wt_pong : memref<{FAT_CHUNK_BF16}xbf16>, 0, {FAT_CHUNK_BF16}) {{bd_id = 1 : i32, next_bd_id = 0 : i32}}
      aie.use_lock(%{tile}_wt_pong_full, Release, {ROWS_PER_COLUMN})
...
def _runtime_sequence() -> str:
    ...
    for column in MAIN_COLUMNS:
        lines.append(_npu_sync(column, 1))
    lines.append("    }")
    return "\n".join(lines)
...
{chr(10).join(flows)}
    ...
    func.func private @emit_seed_sideband(memref<{RECORD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/phase_chain.o"}}
    func.func private @main_emit_phase_record(memref<{RECORD_DWORDS}xi32>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, i32, i32, i32, i32) attributes {{link_with = "{experiment_dir}/phase_chain.o"}}
    func.func private @edge_make_phase_slice(memref<{RECORD_DWORDS}xi32>, memref<{ACT_SLICE_BF16}xbf16>, i32, i32, i32, i32) attributes {{link_with = "{experiment_dir}/phase_chain.o"}}
    func.func private @q4nx_chunk_accum_slice(memref<{CHUNK_BF16}xbf16>, memref<{ACT_SLICE_BF16}xbf16>, i32) attributes {{link_with = "{experiment_dir}/phase_chain.o"}}
    func.func private @q4nx_flush_output(memref<{M_PER_TILE}xbf16>, i32) attributes {{link_with = "{experiment_dir}/phase_chain.o"}}
    func.func private @flush_layer_output_with_header(memref<{OUT_RECORD_BF16}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, memref<{M_PER_TILE}xbf16>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/phase_chain.o"}}
