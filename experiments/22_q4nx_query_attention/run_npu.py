#!/usr/bin/env python3
"""Run exp22 on NPU and compare against the CPU reference."""
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    ...
    if match is None:
        ...
        if len(args) != 5:
            errors.append(f"Expected 5 runtime args, got {len(args)}")

    markers = [
        "q4nx_query_accum_offset",
        "q4nx_query_flush",
        "current_k",
        "current_v",
...
def compile_kernel() -> bool:
    ...
    src = EXPERIMENT_DIR / "q4_query_attention.cc"
    obj = EXPERIMENT_DIR / "q4_query_attention.o"
    cmd = [
        str(clang),
        "-O2",
        "-std=c++20",
    ...
    print("  Compiling q4_query_attention.cc...")
...
def _check_current_cache_write(L: int, before: np.ndarray, after_npu: np.ndarray, current: np.ndarray) -> list[str]:
    ...
    if not np.allclose(after_npu[k_start:k_start + TOKEN_DWORDS], expected_after[k_start:k_start + TOKEN_DWORDS]):
        errors.append("K current token cache write mismatch")
    if not np.allclose(after_npu[v_start:v_start + TOKEN_DWORDS], expected_after[v_start:v_start + TOKEN_DWORDS]):
...
def run_case(L: int, hidden: np.ndarray, packed_weight: np.ndarray) -> bool:
    ...
    current, cache_before, query, expected = layer_reference_after_current_write(L, hidden, packed_weight)
    ...
    result = aie_utils.DefaultNPURuntime.run(handle, [hidden_buf, weight_buf, current_buf, cache_buf, output_buf])
...
def main() -> bool:
    print("=" * 72)
    print("Experiment 22: Q4NX Query Projection + KV Attention")
    print("=" * 72)
    print(f"Hidden dim: {HIDDEN_DIM} bf16, Q chunks: {Q_CHUNKS}")
    print(f"Attention: {TOKEN_DWORDS} dwords per token, {TOKENS_PER_TILE} tokens per tile")
    print()

    hidden, packed_weight, _, _, query = make_case_data()
    ...
    print("=" * 72)
    if all_passed:
        print("SUCCESS: Q4NX query projection feeds KV attention.")
    else:
