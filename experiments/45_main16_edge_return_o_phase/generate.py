"""Generate raw MLIR-AIE for exp45 main16 -> edge -> main16 O phase."""
...
def _main_tile(group: int, row: int) -> str:
    ...
    return f"""
    %{tile}_weight = aie.buffer(%{tile}) {{sym_name = "{tile}_weight"}} : memref<{SHARD_DWORDS}xi32>
...
{_lock_pair(tile, "weight", 0)}
{_lock_pair(tile, "attention", 2)}
{_lock_pair(tile, "record", 4)}
{_lock_pair(tile, "output", 6)}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      func.call @main_o_phase(%{tile}_attention, %{tile}_weight, %{tile}_output, %group_i32, %row_i32)
        : (memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, i32, i32) -> ()
    ...
    ^weight_in:
      aie.use_lock(%{tile}_weight_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{tile}_weight : memref<{SHARD_DWORDS}xi32>, 0, {SHARD_DWORDS}) {{bd_id = 0 : i32}}
      aie.use_lock(%{tile}_weight_full, Release, 1)
...
def _memtile_column(group: int) -> str:
    ...
    for bd_index, row in enumerate(output_order):
      ...
      aie.use_lock(%{tile}_output_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{tile}_output : memref<{COLUMN_DWORDS}xi32>, {row * SHARD_DWORDS}, {SHARD_DWORDS}) {{bd_id = {7 + bd_index} : i32, next_bd_id = {7 + next_bd_index} : i32}}
      aie.use_lock(%{tile}_output_full, Release, 1)
    ...
    ^out{ch}_weight:
      aie.use_lock(%{tile}_weight_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%{tile}_weight : memref<{COLUMN_DWORDS}xi32>, {row * SHARD_DWORDS}, {SHARD_DWORDS}) {{bd_id = {bds[row]} : i32}}
      aie.use_lock(%{tile}_weight_empty, Release, 1)
      aie.next_bd ^out{ch}_weight"""
        )

    return f"""
    %{tile}_weight = aie.buffer(%{tile}) {{sym_name = "{tile}_weight"}} : memref<{COLUMN_DWORDS}xi32>
    %{tile}_output = aie.buffer(%{tile}) {{sym_name = "{tile}_output"}} : memref<{COLUMN_DWORDS}xi32>
    ...
    ^weight_in:
      ...
      aie.dma_bd(%{tile}_weight : memref<{COLUMN_DWORDS}xi32>, 0, {COLUMN_DWORDS}) {{bd_id = 0 : i32}}
...
{chr(10).join(weight_channels)}
    ...
    ^output_drain:
      ...
      aie.dma_bd(%{tile}_output : memref<{COLUMN_DWORDS}xi32>, 0, {COLUMN_DWORDS}) {{bd_id = 34 : i32}}
...
def _runtime_sequence() -> str:
    lines = [
        f"    aie.runtime_sequence(%weights: memref<{TOTAL_DWORDS}xi32>, "
        f"%output: memref<{TOTAL_DWORDS}xi32>) {{"
    ]
    for group, main_col in enumerate(MAIN_COLUMNS):
        offset = group * COLUMN_DWORDS * 4
        lines += [
            _npu_writebd(main_col, 13, COLUMN_DWORDS, offset),
            _npu_address_patch(main_col, 13, 1, offset),
            _npu_push_queue(main_col, "S2MM", 1, 13, issue_token=True),
            _npu_writebd(main_col, 0, COLUMN_DWORDS, offset),
            _npu_address_patch(main_col, 0, 0, offset),
...
{chr(10).join(flows)}
    ...
    func.func private @emit_main_record(memref<{RECORD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/edge_return_o_phase.o"}}
    func.func private @edge_make_attention_row(memref<{RECORD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/edge_return_o_phase.o"}}
    func.func private @main_o_phase(memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, memref<{SHARD_DWORDS}xi32>, i32, i32) attributes {{link_with = "{experiment_dir}/edge_return_o_phase.o"}}
