#!/usr/bin/env python3
"""Run exp66 on real NPU and verify real attention-to-O producer handoff."""
...
def run_on_npu() -> bool:
    print("=" * 78)
    print("Experiment 66: MyLM Real Attention Producer Into O Phase")
    print("=" * 78)
    ...
    print(
        "  O phase activation source: exp65 Q/K/V sideband payloads -> "
        f"L={CONTEXT_LEN} online-softmax weighted-V producer"
    )
    ...
    if mismatch.size == 0:
        print(
            "  PASS: real Qwen3 Q/K/V/O/up/gate/down schedule ran through "
            "packetized MyLM patch queues with real attention-result-to-O handoff."
        )
        return True
