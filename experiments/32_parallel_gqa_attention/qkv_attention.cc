...
void online_head_attention_pair(
    float *query,
    float *kv_buf,
    float *out_buf,
    float *running_max,
    ...
    for (int t = 0; t < valid; t++) {
        ...
        for (int d = 0; d < HEAD_DIM; d++) {
            score += query[d] * kv_buf[t * KV_DWORDS + d];
        }
        score *= scale;
        scores[t] = score;
    ...
    float new_sum = running_sum[0] * old_scale + local_sum;
    for (int d = 0; d < HEAD_DIM; d++) {
        float acc = out_buf[d] * old_scale;
        for (int t = 0; t < valid; t++) {
