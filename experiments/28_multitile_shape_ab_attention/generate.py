"""Generate exp28 multi-tile shape-A/shape-B online attention MLIR."""
...
def _shape_a(num_tiles: int, last_valid: int) -> str:
    return f"""
...
{_lock_pair("shape_a", "sideband", 4)}
    ...
    %shape_a_core = aie.core(%shape_a) {{
      ...
      scf.for %i = %c0 to %num_tiles_idx step %c1 {{
        ...
        func.call @shape_a_tile_sideband(%shape_a_current, %shape_a_history, %shape_a_sideband, %i_i32, %num_tiles_i32, %last_valid_i32)
          : (memref<{CURRENT_DWORDS}xf32>, memref<{HISTORY_DWORDS}xf32>, memref<{SIDEBAND_DWORDS}xf32>, i32, i32, i32) -> ()
...
def _side_sink(num_tiles: int) -> str:
    return f"""
...
{_lock_pair("side_sink", "forward", 4)}
    ...
    %side_sink_core = aie.core(%side_sink) {{
      ...
      func.call @sideband_debug_init(%side_sink_debug)
        : (memref<{SIDEBAND_DEBUG_DWORDS}xf32>) -> ()
      scf.for %i = %c0 to %num_tiles_idx step %c1 {{
        %i_i32 = arith.index_cast %i : index to i32
        aie.use_lock(%side_sink_in_full, AcquireGreaterEqual, 1)
        aie.use_lock(%side_sink_forward_empty, AcquireGreaterEqual, 1)
        func.call @sideband_debug_and_forward_tile(%side_sink_in, %side_sink_debug, %side_sink_forward, %i_i32)
          : (memref<{SIDEBAND_DWORDS}xf32>, memref<{SIDEBAND_DEBUG_DWORDS}xf32>, memref<{SIDEBAND_DWORDS}xf32>, i32) -> ()
...
def _shape_b(num_tiles: int) -> str:
    return f"""
...
{_lock_pair("shape_b", "output", 4)}
    ...
    %shape_b_core = aie.core(%shape_b) {{
      ...
      func.call @shape_b_init(%shape_b_output)
        : (memref<{ATTENTION_OUT_DWORDS}xf32>) -> ()
      scf.for %i = %c0 to %num_tiles_idx step %c1 {{
        aie.use_lock(%shape_b_sideband_full, AcquireGreaterEqual, 1)
        aie.use_lock(%shape_b_history_full, AcquireGreaterEqual, 1)
        func.call @shape_b_accumulate_tile(%shape_b_sideband, %shape_b_history, %shape_b_output)
      ...
      func.call @shape_b_finalize(%shape_b_output)
        : (memref<{ATTENTION_OUT_DWORDS}xf32>) -> ()
...
def _runtime_sequence(num_tiles: int) -> str:
    history_total = num_tiles * HISTORY_SOURCE_DWORDS
    attention_offset = SIDEBAND_DEBUG_DWORDS * 4
    return "\n".join(
        [
            f"    aie.runtime_sequence(%current: memref<{CURRENT_DWORDS}xf32>, "
            f"%k_history: memref<{history_total}xf32>, "
            f"%v_history: memref<{history_total}xf32>, "
            ...
            _npu_writebd(0, 0, history_total, 0),
            _npu_address_patch(0, 0, 1, 0),
            _npu_push_queue(0, "MM2S", 0, 0),
            _npu_writebd(7, 0, history_total, 0),
...
def generate_mlir(context_len: int) -> str:
  ...
  aie.device(npu2) {{
    ...
    func.func private @shape_a_tile_sideband(memref<{CURRENT_DWORDS}xf32>, memref<{HISTORY_DWORDS}xf32>, memref<{SIDEBAND_DWORDS}xf32>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/attention_kernels.o"}}
    func.func private @sideband_debug_init(memref<{SIDEBAND_DEBUG_DWORDS}xf32>) attributes {{link_with = "{experiment_dir}/attention_kernels.o"}}
    func.func private @sideband_debug_and_forward_tile(memref<{SIDEBAND_DWORDS}xf32>, memref<{SIDEBAND_DEBUG_DWORDS}xf32>, memref<{SIDEBAND_DWORDS}xf32>, i32) attributes {{link_with = "{experiment_dir}/attention_kernels.o"}}
    func.func private @shape_b_init(memref<{ATTENTION_OUT_DWORDS}xf32>) attributes {{link_with = "{experiment_dir}/attention_kernels.o"}}
    func.func private @shape_b_accumulate_tile(memref<{SIDEBAND_DWORDS}xf32>, memref<{HISTORY_DWORDS}xf32>, memref<{ATTENTION_OUT_DWORDS}xf32>) attributes {{link_with = "{experiment_dir}/attention_kernels.o"}}
    func.func private @shape_b_finalize(memref<{ATTENTION_OUT_DWORDS}xf32>) attributes {{link_with = "{experiment_dir}/attention_kernels.o"}}

{_history_ring("mem0", 0)}
{_history_ring("mem7", 0)}
{_current_source()}
{_shape_a(num_tiles, last_valid)}
{_side_sink(num_tiles)}
{_shape_b(num_tiles)}
{_runtime_sequence(num_tiles)}
  }}
}}
"""
...
if __name__ == "__main__":
    ...
    print(generate_mlir(int(sys.argv[1]) if len(sys.argv) > 1 else 31))
