#!/usr/bin/env python3
"""Run exp50 on real NPU and verify edge-slice replay -> Q4NX O phase."""
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    expected_counts = {
        ...
        "func.call @q4nx_flush_output": len(MAIN_COLUMNS) * ROWS_PER_COLUMN,
    }
    errors: list[str] = []
...
def run_on_npu() -> bool:
    print("=" * 78)
    print("Experiment 50: Edge 512-Dword Slice Replay To Q4NX O Phase")
    print("=" * 78)
    ...
    print(f"  edge replay: {NUM_CHUNKS} x {ACT_SLICE_BF16} bf16 slices = {O_INPUT_DIM // 2} dwords")
    ...
    print(f"  group0 slice3[0:8]: {attention_slice_from_record(0, 0, record, 3)[:8].tolist()}")
    ...
    if mismatch.size == 0:
        print("  PASS: edge 512-dword shard is replayed as 4 x 128-dword slices into Q4NX O phase.")
        return True
    ...
    for group in range(len(MAIN_COLUMNS)):
        ...
        for row in range(ROWS_PER_COLUMN):
            ...
            print(
                f"    row {row}: expected={expected[start:end].tolist()} "
                f"got={got[start:end].tolist()}"
            )
