#include <stdint.h>

namespace {

constexpr int NUM_Q_HEADS = 4;
constexpr int HEAD_DIM = 128;
constexpr int QUERY_DWORDS = NUM_Q_HEADS * HEAD_DIM;
constexpr int TOKENS_PER_TILE = 16;
constexpr int DIM_GROUPS = 32;
constexpr int GROUP_DWORDS = 4;
constexpr int RESHAPED_TILE_DWORDS = DIM_GROUPS * TOKENS_PER_TILE * GROUP_DWORDS;
constexpr int OUTPUT_DWORDS = QUERY_DWORDS;

static inline float max2(float a, float b) {
    return a > b ? a : b;
}

static inline float approx_exp(float x) {
    if (x <= -16.0f) {
        return 0.0f;
    }
    if (x > 0.0f) {
        x = 0.0f;
    }
    float y = 1.0f + x * 0.015625f;
    y *= y;
    y *= y;
    y *= y;
    y *= y;
    y *= y;
    y *= y;
    return y;
}

static inline int reshaped_index(int dim, int token) {
    int dim_group = dim / GROUP_DWORDS;
    int lane = dim - dim_group * GROUP_DWORDS;
    return (dim_group * TOKENS_PER_TILE + token) * GROUP_DWORDS + lane;
}

} // namespace

extern "C" {

void init_attention_state(float *out_buf, float *running_max, float *running_sum) {
    for (int i = 0; i < OUTPUT_DWORDS; i++) {
        out_buf[i] = 0.0f;
    }
    for (int h = 0; h < NUM_Q_HEADS; h++) {
        running_max[h] = -3.4028234663852886e38f;
        running_sum[h] = 0.0f;
    }
}

void online_attention_reshaped(
    float *query,
    float *k_tile,
    float *v_tile,
    float *out_buf,
    float *running_max,
    float *running_sum,
    int32_t tile_idx,
    int32_t num_tiles,
    int32_t last_valid
) {
    const float scale = 0.08838834764831845f;
    int valid = (tile_idx < num_tiles - 1) ? TOKENS_PER_TILE : last_valid;
    float scores[TOKENS_PER_TILE];
    float weights[TOKENS_PER_TILE];

    for (int h = 0; h < NUM_Q_HEADS; h++) {
        int q_base = h * HEAD_DIM;
        float local_max = -3.4028234663852886e38f;
        for (int t = 0; t < valid; t++) {
            float score = 0.0f;
            for (int d = 0; d < HEAD_DIM; d++) {
                score += query[q_base + d] * k_tile[reshaped_index(d, t)];
            }
            score *= scale;
            scores[t] = score;
            local_max = max2(local_max, score);
        }

        float old_max = running_max[h];
        float new_max = max2(old_max, local_max);
        float old_scale = running_sum[h] == 0.0f ? 0.0f : approx_exp(old_max - new_max);

        float local_sum = 0.0f;
        for (int t = 0; t < valid; t++) {
            float weight = approx_exp(scores[t] - new_max);
            weights[t] = weight;
            local_sum += weight;
        }

        float new_sum = running_sum[h] * old_scale + local_sum;
        for (int d = 0; d < HEAD_DIM; d++) {
            float acc = out_buf[q_base + d] * old_scale;
            for (int t = 0; t < valid; t++) {
                acc += weights[t] * v_tile[reshaped_index(d, t)];
            }
            out_buf[q_base + d] = acc;
        }

        running_max[h] = new_max;
        running_sum[h] = new_sum;
    }
}

void finalize_attention(float *out_buf, float *running_sum) {
    for (int h = 0; h < NUM_Q_HEADS; h++) {
        float denom = running_sum[h];
        int out_base = h * HEAD_DIM;
        for (int d = 0; d < HEAD_DIM; d++) {
            out_buf[out_base + d] = denom == 0.0f ? 0.0f : out_buf[out_base + d] / denom;
        }
    }
}

} // extern "C"
