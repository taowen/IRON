"""CPU reference for exp33 row1 KV reshape attention."""
...
def make_query(seed: int = 33) -> np.ndarray:
    ...
    for tile in range(num_tiles):
        ...
        for head in range(NUM_Q_HEADS):
            ...
            for token in range(valid):
                ...
                for dim in range(HEAD_DIM):
                    score += query[q_base + dim] * k_tile[reshaped_index(dim, token)]
                scores[token] = score * scale

            local_max = np.max(scores)
            new_max = max(running_max[head], local_max)
            old_scale = np.float32(0.0) if running_sum[head] == 0 else approx_exp(running_max[head] - new_max)
            ...
            for token in range(valid):
                for dim in range(HEAD_DIM):
                    out[q_base + dim] += weights[token] * v_tile[reshaped_index(dim, token)]
