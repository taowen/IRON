"""
Experiment 20: online-softmax attention over the current-write KV path.

Exp19 proved the cache boundary with an int32 dot/reduce kernel.  Exp20 keeps
the same current-token write + rounded four-plane scan, but replaces the kernel
with tiled online softmax:
- each worker owns four local heads
- running max and running sum stay core-local across history tiles
- output is rescaled when a later tile raises the running max
- final output is the normalized attention value, not an unnormalized sum
"""
...
def _worker_core(name: str, head_offset: int, num_tiles: int, last_valid: int) -> str:
    return f"""    %{name}_core = aie.core(%{name}) {{
      ...
      scf.for %i = %c0 to %num_tiles_idx step %c1 {{
        ...
        scf.if %is_pong {{
          func.call @online_softmax_attention(%{name}_k_pong, %{name}_v_pong, %{name}_out, %{name}_running_max, %{name}_running_sum, %i_i32, %num_tiles_i32, %last_valid_i32, %head_offset_i32)
            : (memref<{PLANE_TILE_DWORDS}xf32>, memref<{PLANE_TILE_DWORDS}xf32>, memref<{OUTPUT_DWORDS_PER_WORKER}xf32>, memref<{NUM_KV_HEADS_PER_GROUP}xf32>, memref<{NUM_KV_HEADS_PER_GROUP}xf32>, i32, i32, i32, i32) -> ()
        }} else {{
          func.call @online_softmax_attention(%{name}_k_ping, %{name}_v_ping, %{name}_out, %{name}_running_max, %{name}_running_sum, %i_i32, %num_tiles_i32, %last_valid_i32, %head_offset_i32)
            : (memref<{PLANE_TILE_DWORDS}xf32>, memref<{PLANE_TILE_DWORDS}xf32>, memref<{OUTPUT_DWORDS_PER_WORKER}xf32>, memref<{NUM_KV_HEADS_PER_GROUP}xf32>, memref<{NUM_KV_HEADS_PER_GROUP}xf32>, i32, i32, i32, i32) -> ()
        }}
...
{_worker_buffers_and_locks("w1")}
    ...
    func.func private @online_softmax_attention(memref<{PLANE_TILE_DWORDS}xf32>, memref<{PLANE_TILE_DWORDS}xf32>, memref<{OUTPUT_DWORDS_PER_WORKER}xf32>, memref<{NUM_KV_HEADS_PER_GROUP}xf32>, memref<{NUM_KV_HEADS_PER_GROUP}xf32>, i32, i32, i32, i32) attributes {{link_with = "{experiment_dir}/kv_attention.o"}}
