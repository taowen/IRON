#!/usr/bin/env python3
"""Run experiment 20 on NPU and compare against CPU online softmax."""
...
from reference import apply_current_to_cache, attention_reference_after_current_write
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    ...
    for marker in ["current_k", "current_v", "running_max", "running_sum", "online_softmax_attention"]:
...
def main() -> bool:
    print("=" * 72)
    print("Experiment 20: Online Softmax KV Attention")
    print("=" * 72)
    print(f"Plane tile: 16 tokens x {NUM_KV_HEADS_PER_GROUP} heads x {HEAD_DIM} dims")
    print(f"State: running_max/running_sum per head, output accumulator per head")
    print()
    ...
    print("=" * 72)
    if all_passed:
        print("SUCCESS: tiled online softmax attention verified.")
    else:
