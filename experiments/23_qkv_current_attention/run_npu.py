#!/usr/bin/env python3
"""Run exp23 on NPU and compare against the CPU reference."""
...
def check_mlir_structure(mlir_text: str, num_tiles: int) -> list[str]:
    errors: list[str] = []
    expected_counts = {
        "aiex.npu.writebd": 10 + 2 * num_tiles,
        "aiex.npu.address_patch": 10 + 2 * num_tiles,
        "aiex.npu.push_queue": 10 + 2 * num_tiles,
        "aiex.npu.sync": 2,
        "aie.flow": 5,
        "aie.memtile_dma": 1,
        "aie.core(": 1,
...
def main() -> bool:
    print("=" * 72)
    print("Experiment 23: Q/K/V Projection + Cache-Backed Attention")
    print("=" * 72)
    ...
    print("=" * 72)
    if all_passed:
        print("SUCCESS: Q/K/V projection writes cache and feeds rounded KV attention.")
    else:
