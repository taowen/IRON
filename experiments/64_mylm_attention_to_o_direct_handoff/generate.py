"""Generate raw MLIR-AIE for exp64 attention-to-O direct handoff."""
...
{_lock_pair(tile, "act", 0, init_empty=2)}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      scf.for %chunk = %c0 to %num_chunks step %c1 {{
        ...
        scf.if %is_pong {{
          func.call @edge_replay_attention_slice(%{tile}_act_pong, %group_i32, %row_i32, %chunk_i32)
            : (memref<{ACT_SLICE_BF16}xbf16>, i32, i32, i32) -> ()
        }} else {{
          func.call @edge_replay_attention_slice(%{tile}_act_ping, %group_i32, %row_i32, %chunk_i32)
            : (memref<{ACT_SLICE_BF16}xbf16>, i32, i32, i32) -> ()
        }}
...
{chr(10).join(flows)}
    ...
    func.func private @edge_replay_attention_slice(memref<{ACT_SLICE_BF16}xbf16>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/projection_nblock.o"}}
