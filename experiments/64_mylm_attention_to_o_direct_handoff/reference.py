"""CPU reference for exp64 attention-to-O direct handoff."""
...
def attention_slice(group: int, row: int, chunk: int) -> np.ndarray:
    del group, row
    base = chunk * K_CHUNK
    values = []
    for idx in range(K_CHUNK):
        values.append(attention_output_value(base + idx))
...
def expected_tile_record(packed: np.ndarray, group: int, row: int) -> np.ndarray:
    ...
    for chunk in range(K_CHUNKS):
        accum += q4nx_chunk_matvec(
            ...
            attention_slice(group, row, chunk),
        )
