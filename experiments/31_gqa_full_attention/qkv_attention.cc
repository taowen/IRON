...
constexpr int QUERY_DWORDS = NUM_Q_HEADS * HEAD_DIM;
constexpr int KV_DWORDS = HEAD_DIM;
constexpr int TOKENS_PER_TILE = 16;
constexpr int KV_PLANE_TILE_DWORDS = KV_DWORDS * TOKENS_PER_TILE;
...
template <int OutputDwords>
static inline void q4nx_project_chunk_impl(
...
void zero_query(float *dst) {
    for (int i = 0; i < QUERY_DWORDS; i++) {
        dst[i] = 0.0f;
    }
}

void zero_kv(float *dst) {
    for (int i = 0; i < KV_DWORDS; i++) {
        dst[i] = 0.0f;
    }
}

void q4nx_project_query_chunk(
    bfloat16 *packed_chunk,
    bfloat16 *activation,
    float *output,
    int32_t output_offset,
    ...
    q4nx_project_chunk_impl<QUERY_DWORDS>(packed_chunk, activation, output, output_offset, act_offset);
}

void q4nx_project_kv_chunk(
    bfloat16 *packed_chunk,
    bfloat16 *activation,
    float *output,
    int32_t output_offset,
    ...
    q4nx_project_chunk_impl<KV_DWORDS>(packed_chunk, activation, output, output_offset, act_offset);
}

void copy_kv(float *src, float *dst) {
    for (int i = 0; i < KV_DWORDS; i++) {
        dst[i] = src[i];
    }
}
...
void online_gqa_attention_pair(
    float *query,
    float *kv_buf,
    float *out_buf,
    float *running_max,
    ...
    for (int h = 0; h < NUM_Q_HEADS; h++) {
        ...
        for (int t = 0; t < valid; t++) {
            ...
            for (int d = 0; d < HEAD_DIM; d++) {
                score += query[q_base + d] * kv_buf[t * KV_DWORDS + d];
            }
            score *= scale;
            scores[t] = score;
        ...
        for (int d = 0; d < HEAD_DIM; d++) {
            float acc = out_buf[q_base + d] * old_scale;
            for (int t = 0; t < valid; t++) {
                acc += weights[t] * kv_buf[KV_PLANE_TILE_DWORDS + t * KV_DWORDS + d];
            }
            out_buf[q_base + d] = acc;
        }
