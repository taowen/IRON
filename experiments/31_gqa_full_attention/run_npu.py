#!/usr/bin/env python3
"""Run exp31 on NPU and compare against the CPU reference."""
...
def check_mlir_structure(mlir_text: str, num_tiles: int) -> list[str]:
    ...
    markers = [
        "q4nx_project_query_chunk",
        "q4nx_project_kv_chunk",
        "online_gqa_attention_pair",
        f"memref<{QUERY_DWORDS}xf32>",
    ...
    if mlir_text.count(f"buffer_length = {Q_PHASE_I32} : i32") != 1:
        errors.append("Expected one full-K Q phase weight BD")
    if mlir_text.count(f"buffer_length = {KV_PHASE_I32} : i32") != 2:
        errors.append("Expected two full-K K/V phase weight BDs")
    return errors
...
def main() -> bool:
    print("=" * 72)
    print("Experiment 31: Full GQA Attention Group")
    print("=" * 72)
    print(f"Hidden dim: {HIDDEN_DIM} bf16, Q4 chunks per rowblock: {Q4_CHUNKS}")
    print(f"Q phase BD length: {Q_PHASE_I32} i32, K/V phase BD length: {KV_PHASE_I32} i32")
    print(f"Query dwords: {QUERY_DWORDS}, KV dwords: {KV_DWORDS}, KV tile pair: {KV_TILE_DWORDS} f32")
    print()
    ...
    print("=" * 72)
    if all_passed:
        print("SUCCESS: full GQA group projection writes cache and feeds attention.")
    else:
