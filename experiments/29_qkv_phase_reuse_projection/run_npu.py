#!/usr/bin/env python3
"""
Experiment 29: Q/K/V Phase-Reuse Projection Contract on NPU.
...
6. Activation hold (core holds activation in local memory across Q/K/V phases)
7. MyLM-style phase reuse at projection level: Q, K, and V run on the same tiles
"""
...
from generate import (
    ...
    TOTAL_WT_I32, ACT_I32, OUT_TOTAL_I32, PROJECTION_NAMES,
)
...
def check_mlir_structure(mlir_text: str) -> list:
    ...
    expected_writebd = NUM_COLS * (1 + NUM_PROJECTIONS + NUM_PROJECTIONS * ROWS_PER_COL)
    ...
    bd_ids = [int(v) for v in re.findall(r"aiex\.npu\.writebd \{bd_id = ([0-9]+) : i32", mlir_text)]
    max_bd = max(bd_ids) if bd_ids else -1
    if max_bd > 15:
        errors.append(f"Shim/core runtime BD id exceeds hardware limit: max bd_id={max_bd}")

    expected_sync = NUM_COLS * ROWS_PER_COL
    n_sync = len(re.findall(r"aiex\.npu\.sync", mlir_text))
    if n_sync != expected_sync:
        errors.append(f"Expected {expected_sync} sync, got {n_sync}")
    ...
    # Q/K/V must time-reuse the same hidden load. There should still be only
    # one activation DMA task per column, not one per projection phase.
    n_act_patches = len(re.findall(r"address_patch \{addr = [0-9]+ : ui32, arg_idx = 1", mlir_text))
    if n_act_patches != NUM_COLS:
        errors.append(f"Expected one activation address_patch per column ({NUM_COLS}), got {n_act_patches}")

    n_weight_patches = len(re.findall(r"address_patch \{addr = [0-9]+ : ui32, arg_idx = 0", mlir_text))
    expected_weight_patches = NUM_COLS * NUM_PROJECTIONS
    if n_weight_patches != expected_weight_patches:
        errors.append(f"Expected one weight address_patch per column per projection phase ({expected_weight_patches}), got {n_weight_patches}")
...
def run_on_npu():
    ...
    print(f"  Projection phases: {' -> '.join(PROJECTION_NAMES)}")
    ...
    print(f"  Ref output[0:4] (col0_r0_Q): {ref_output[:4]}")
    print(f"  Ref output[32:36] (col0_r0_K): {ref_output[32:36]}")
    print(f"  Ref output[64:68] (col0_r0_V): {ref_output[64:68]}")
    ...
    print(f"  NPU output[0:4] (col0_r0_Q): {npu_output[:4]}")
    print(f"  NPU output[32:36] (col0_r0_K): {npu_output[32:36]}")
    print(f"  NPU output[64:68] (col0_r0_V): {npu_output[64:68]}")
    ...
    for col in range(NUM_COLS):
        for row in range(ROWS_PER_COL):
            for proj in range(NUM_PROJECTIONS):
                ...
                print(f"    Col{col} Row{row} {PROJECTION_NAMES[proj]}: max_err={tile_err:.4f}")

    # Q4/bf16 accumulated projections have a few ULPs of drift after K=4096.
    rel_tol = 0.15
    abs_tol = 2.0
    ...
    if num_errors == 0:
        ...
        for idx in error_indices[:12]:
            ...
            elem = rem % M_PER_TILE
            print(f"    [{idx}] col{col}_r{row}_{PROJECTION_NAMES[proj]}_e{elem}: ref={ref_f32[idx]:.4f} npu={npu_f32[idx]:.4f} err={abs_err[idx]:.4f}")

    print()
    print("=" * 70)
    if num_errors == 0:
        print("SUCCESS: Q/K/V phase-reuse projection produces CORRECT output!")
        print(f"  {NUM_TILES} tiles ({NUM_COLS} cols x {ROWS_PER_COL} rows) x Q/K/V = {TOTAL_OUTPUT} outputs verified")
        ...
        print(f"  Activation hold across Q/K/V phases (no DDR re-fetch)")
        print(f"  Phase reuse: one long weight stream per projection phase ({NUM_CHUNKS} fat chunks each)")
...
if __name__ == "__main__":
    print("=" * 70)
    print("Experiment 29: Q/K/V Phase-Reuse Projection")
    print("  Same physical tiles run Q -> K -> V")
    print("=" * 70)
    print()
