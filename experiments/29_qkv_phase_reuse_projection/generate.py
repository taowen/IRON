"""
Experiment 29: Q/K/V Phase-Reuse Projection Contract
...
- The same physical tiles run Q, then K, then V phases over time
"""
...
# Weight BO in i32: one phase-ordered Q/K/V stream per column.
...
def _npu_sync(column, direction, channel=0):
    return f"      aiex.npu.sync {{channel = {channel} : i32, column = {column} : i32, column_num = 1 : i32, direction = {direction} : i32, row = 0 : i32, row_num = 1 : i32}}"
...
def _core_program(col, row, prefix):
    ...
    return f"""    %core_{prefix} = aie.core(%{prefix}) {{
      ...
      // Acquire activation once and hold it across Q, K, and V phases.
      ...
      // Release activation after all Q/K/V phases.
...
def generate_mlir() -> str:
    ...
    phase_wt_bytes = NUM_CHUNKS * FAT_CHUNK_BF16 * 2
    ...
    for col in range(NUM_COLS):
        ...
        # Output BDs are armed before weight batches.  Otherwise a sync after a
        # completed Q phase can deadlock: the cores may be blocked trying to
        # drain Q output while the host is waiting before queuing K weights.
        output_bd_base = 1 + NUM_PROJECTIONS
        for proj in range(NUM_PROJECTIONS):
            for row in range(ROWS_PER_COL):
                out_off = (col * ROWS_PER_COL * NUM_PROJECTIONS + row * NUM_PROJECTIONS + proj) * OUT_BF16 * 2
                bd_id = output_bd_base + row * NUM_PROJECTIONS + proj
                is_last_for_row = (proj == NUM_PROJECTIONS - 1)
                rt_lines.append(_npu_writebd(col, bd_id, OUT_BF16 // 2, out_off))
                rt_lines.append(_npu_address_patch(col, bd_id, 2, out_off))
                rt_lines.append(_npu_push_queue(col, "S2MM", row, bd_id, issue_token=is_last_for_row))

        # Weights: one full-K patch per projection phase.  This mirrors the
        # MyLM weight patch granularity for a 64-row shard: 0x28000 bytes for
        # K=4096.  The static row1 ring splits the long stream into 16 fat
        # chunks; runtime does not wait between projection phases.
        for proj in range(NUM_PROJECTIONS):
            bd_id = 1 + proj
            wt_off = col * per_col_wt_bytes + proj * phase_wt_bytes
            rt_lines.append(_npu_writebd(col, bd_id, phase_wt_bytes // 4, wt_off))
            rt_lines.append(_npu_address_patch(col, bd_id, 0, wt_off))
            rt_lines.append(_npu_push_queue(col, "MM2S", 1, bd_id, issue_token=True))

    for col in range(NUM_COLS):
        for row in range(ROWS_PER_COL):
            rt_lines.append(_npu_sync(col, 0, row))
    rt_lines.append("    }")
    runtime_sequence = "\n".join(rt_lines)
