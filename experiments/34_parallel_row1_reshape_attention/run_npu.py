#!/usr/bin/env python3
"""Run exp34 on NPU and compare against the CPU reference."""
...
def check_mlir_structure(mlir_text: str, num_tiles: int) -> list[str]:
    errors: list[str] = []
    expected_counts = {
        "aiex.npu.writebd": NUM_COLS * (2 * num_tiles + 2),
        "aiex.npu.address_patch": NUM_COLS * (2 * num_tiles + 2),
        "aiex.npu.push_queue": NUM_COLS * (2 * num_tiles + 2),
    ...
    if mlir_text.count(reshape_dims) != NUM_COLS * 4:
        errors.append("Expected K/V ping-pong row1 reshape dimensions in each column")
...
def main() -> bool:
    print("=" * 72)
    print("Experiment 34: Parallel Row1 KV Reshape Attention")
    print("=" * 72)
    ...
    print("=" * 72)
    if all_passed:
        print("SUCCESS: parallel row1 reshape feeds four GQA attention heads.")
    else:
