#!/usr/bin/env python3
"""Run exp56 on real NPU and verify the linked Qwen3 patch schedule."""
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    weight_batches = (TOTAL_LOGICAL_BLOCKS + WEIGHT_BD_BATCH - 1) // WEIGHT_BD_BATCH
    runtime_descriptor_count = len(MAIN_COLUMNS) + len(MAIN_COLUMNS) * TOTAL_LOGICAL_BLOCKS
    push_count = len(MAIN_COLUMNS) + len(MAIN_COLUMNS) * weight_batches
    sync_count = len(MAIN_COLUMNS) + len(MAIN_COLUMNS) * weight_batches
    linked_bd_count = len(MAIN_COLUMNS) * (TOTAL_LOGICAL_BLOCKS - weight_batches)
    expected_counts = {
...
def run_on_npu() -> bool:
    print("=" * 78)
    print("Experiment 56: MyLM-Style Linked Qwen3 Patch Schedule")
    print("=" * 78)
    ...
    print(f"  runtime weight source: linked BD batches, {WEIGHT_BD_BATCH} BDs per batch")
    ...
    if mismatch.size == 0:
        print(
            "  PASS: real Qwen3 Q/K/V/O/up/gate/down patch schedule ran through "
            "MyLM-style linked BD batches on the same main16/edge16 fabric."
        )
        return True
