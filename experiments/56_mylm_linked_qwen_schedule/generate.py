"""Generate raw MLIR-AIE for exp56 MyLM-style linked Qwen3 patch schedule."""
...
PHASE_BLOCK_WEIGHT_BF16 = tuple(PHASE_CHUNKS[phase] * FAT_CHUNK_BF16 for phase in range(NUM_PHASES))
...
def _main_phase_loop(tile: str, phase: int, blocks: int, chunks: int, is_last_phase: bool) -> str:
    final_branch = ""
    if is_last_phase:
        final_branch = f"""
...
def _main_tile(group: int, row: int) -> str:
    ...
    for phase, blocks in enumerate(PHASE_BLOCKS):
        phase_loops.append(_main_phase_loop(tile, phase, blocks, PHASE_CHUNKS[phase], phase == NUM_PHASES - 1))
...
def _runtime_sequence() -> str:
    ...
    logical_blocks: list[tuple[int, int]] = []
    phase_base = 0
    for phase in range(NUM_PHASES):
        block_size = PHASE_BLOCK_WEIGHT_BF16[phase]
        for block in range(PHASE_BLOCKS[phase]):
            logical_blocks.append((phase, phase_base + block * block_size))
        phase_base += PHASE_WEIGHT_BF16[phase]

    for batch_start in range(0, len(logical_blocks), WEIGHT_BD_BATCH):
        batch = logical_blocks[batch_start:batch_start + WEIGHT_BD_BATCH]
        for bd_slot, (phase, block_offset_bf16) in enumerate(batch):
            bd_id = WEIGHT_BD_IDS[bd_slot]
            next_bd = WEIGHT_BD_IDS[bd_slot + 1] if bd_slot + 1 < len(batch) else 0
            block_bf16 = PHASE_BLOCK_WEIGHT_BF16[phase]
            for group, column in enumerate(MAIN_COLUMNS):
                weight_offset = (group * COLUMN_WEIGHT_BF16 + block_offset_bf16) * 2
                lines += [
                    _npu_writebd(
                        column,
                        bd_id,
