#!/usr/bin/env python3
"""Run exp30 on NPU and compare against the CPU reference."""
...
def check_mlir_structure(mlir_text: str, num_tiles: int) -> list[str]:
    errors: list[str] = []
    expected_counts = {
        "aiex.npu.writebd": 7 + num_tiles,
        "aiex.npu.address_patch": 7 + num_tiles,
        "aiex.npu.push_queue": 7 + num_tiles,
        "aiex.npu.sync": 2,
        "aie.flow": 5,
        "aie.memtile_dma": 1,
        "aie.core(": 1,
    ...
    expected_phase_len = f"buffer_length = {ROWBLOCK_I32} : i32"
    if mlir_text.count(expected_phase_len) != 3:
        errors.append("Expected three full-K phase-sized weight BDs")
    return errors
...
def _check_current_cache_write(
    ...
    errors: list[str] = []
    if not np.allclose(
        after_npu[k_offset:k_offset + TOKEN_DWORDS],
        expected_after[k_offset:k_offset + TOKEN_DWORDS],
        rtol=1e-3,
        atol=5e-3,
    ):
        errors.append("K current token cache write mismatch")
    if not np.allclose(
        after_npu[v_offset:v_offset + TOKEN_DWORDS],
        expected_after[v_offset:v_offset + TOKEN_DWORDS],
        rtol=1e-3,
        atol=5e-3,
    ):
...
def build_case(L: int) -> tuple[Path, Path]:
...
def run_case(L: int, hidden: np.ndarray, packed_weight: np.ndarray) -> bool:
    ...
    xclbin_path, insts_path = build_case(L)
    kernel = NPUKernel(
    ...
    query, current_k, current_v, cache_before, expected = attention_reference_after_current_write(L, hidden, packed_weight)
...
def main() -> bool:
    print("=" * 72)
    print("Experiment 30: Full-K Q/K/V Projection -> Cache-Backed Attention")
    print("=" * 72)
    print(f"Hidden dim: {HIDDEN_DIM} bf16, Q4 chunks per phase: {Q4_CHUNKS}")
    print(f"Weight phase BD length: {ROWBLOCK_I32} i32")
    print(f"KV tile pair: {KV_TILE_DWORDS} f32, output token dwords: {TOKEN_DWORDS}")
    print()
    ...
    print("=" * 72)
    if all_passed:
        print("SUCCESS: full-K Q/K/V projection writes cache and feeds attention.")
    else:
