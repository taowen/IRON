"""Generate raw MLIR-AIE for exp30 full-K Q/K/V projection into attention."""
...
def _runtime_sequence(L: int, num_tiles: int) -> str:
    ...
    current_tile = (L - 1) // TOKENS_PER_TILE
    current_token = (L - 1) % TOKENS_PER_TILE
    current_tile_base = current_tile * KV_TILE_DWORDS
    current_k_bytes = (current_tile_base + current_token * TOKEN_DWORDS) * 4
    current_v_bytes = (current_tile_base + PLANE_TILE_DWORDS + current_token * TOKEN_DWORDS) * 4
    ...
    lines += [
        ...
        _npu_writebd(0, 13, TOKEN_DWORDS, current_k_bytes),
        _npu_address_patch(0, 13, 2, current_k_bytes),
        _npu_push_queue(0, "S2MM", 0, 13),
        _npu_writebd(0, 14, TOKEN_DWORDS, current_v_bytes),
        _npu_address_patch(0, 14, 2, current_v_bytes),
        _npu_push_queue(0, "S2MM", 0, 14, issue_token=True),
    ]

    for projection in range(PROJECTIONS):
        offset = projection * ROWBLOCK_BF16 * 2
        bd_id = 1 + projection
        lines += [
            _npu_writebd(0, bd_id, ROWBLOCK_I32, offset),
            _npu_address_patch(0, bd_id, 1, offset),
            _npu_push_queue(0, "MM2S", 1, bd_id, issue_token=True),
        ]

    lines.append(_npu_sync(0, 0))

    for tile in range(num_tiles):
        bd_id = tile
        offset = tile * KV_TILE_DWORDS * 4
        lines += [
            _npu_writebd(0, bd_id, KV_TILE_DWORDS, offset),
            _npu_address_patch(0, bd_id, 2, offset),
            _npu_push_queue(0, "MM2S", 0, bd_id),
        ]
...
def _project_call(chunk_index: int, target: str, act_offset: int) -> str:
    wt = "w0_wt_pong" if chunk_index % 2 else "w0_wt_ping"
    return f"""
      aie.use_lock(%w0_wt_full, AcquireGreaterEqual, 1)
      func.call @q4nx_project_chunk_accum(%{wt}, %w0_hidden, %{target}, %act{act_offset}_i32)
        : (memref<{CHUNK_BF16}xbf16>, memref<{HIDDEN_DIM}xbf16>, memref<{TOKEN_DWORDS}xf32>, i32) -> ()
...
def _projection_sequence() -> str:
    ...
    for target in ["w0_query", "w0_current_k", "w0_current_v"]:
        for chunk in range(Q4_CHUNKS):
            lines.append(_project_call(chunk_index, target, chunk * Q4_K_CHUNK))
            chunk_index += 1
    return "\n".join(lines)
...
def _worker_core(num_tiles: int, last_valid: int) -> str:
    act_constants = "\n".join(
        f"      %act{chunk * Q4_K_CHUNK}_i32 = arith.constant {chunk * Q4_K_CHUNK} : i32"
        for chunk in range(Q4_CHUNKS)
    )
...
{_projection_sequence()}
      ...
      scf.for %i = %c0 to %num_tiles_idx step %c1 {{
        ...
        func.call @online_softmax_attention_pair(%w0_query, %w0_kv_tile, %w0_out, %w0_running_max, %w0_running_sum, %i_i32, %num_tiles_i32, %last_valid_i32)
          : (memref<{TOKEN_DWORDS}xf32>, memref<{KV_TILE_DWORDS}xf32>, memref<{OUTPUT_DWORDS}xf32>, memref<{NUM_HEADS}xf32>, memref<{NUM_HEADS}xf32>, i32, i32, i32) -> ()
...
{_worker_buffers_and_locks()}
    ...
    func.func private @q4nx_project_chunk_accum(memref<{CHUNK_BF16}xbf16>, memref<{HIDDEN_DIM}xbf16>, memref<{TOKEN_DWORDS}xf32>, i32) attributes {{link_with = "{experiment_dir}/qkv_attention.o"}}
    ...
    func.func private @online_softmax_attention_pair(memref<{TOKEN_DWORDS}xf32>, memref<{KV_TILE_DWORDS}xf32>, memref<{OUTPUT_DWORDS}xf32>, memref<{NUM_HEADS}xf32>, memref<{NUM_HEADS}xf32>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/qkv_attention.o"}}
