...
void probe_tile(float *k_buf, float *v_buf, float *out_buf, int32_t tile_idx) {
    int base = tile_idx * 8;
    float k_sum = 0.0f;
    float v_sum = 0.0f;
    float k_bad = 0.0f;
    float v_bad = 0.0f;
    for (int i = 0; i < TOKENS_PER_TILE * TOKEN_DWORDS; i++) {
        float k = k_buf[i];
        float v = v_buf[i];
        if (k != k) {
            k_bad += 1.0f;
    ...
    out_buf[base + 0] = k_sum;
    out_buf[base + 1] = v_sum;
    out_buf[base + 2] = k_bad;
    out_buf[base + 3] = v_bad;
    out_buf[base + 4] = k_buf[0];
    out_buf[base + 5] = k_buf[32];
    out_buf[base + 6] = v_buf[0];
    out_buf[base + 7] = v_buf[32];
}
