#!/usr/bin/env python3
"""Run exp47 on real NPU and verify main16 multi-phase O/gate/up/down replay."""
...
from generate import (
    COLUMN_OUTPUT_DWORDS,
    COLUMN_WEIGHT_DWORDS,
    RECORD_DWORDS,
    ROWS_PER_COLUMN,
    SHARD_DWORDS,
    TILE_WEIGHT_DWORDS,
...
def compile_kernel() -> None:
    ...
    src = EXPERIMENT_DIR / "multiphase_ffn_replay.cc"
    obj = EXPERIMENT_DIR / "multiphase_ffn_replay.o"
    cmd = [
        str(clang),
        "-O2",
        "-std=c++20",
    ...
    print("  Compiling multiphase_ffn_replay.cc...")
    run_command(cmd)
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    expected_counts = {
        ...
        "func.call @main_gate_phase": 16,
        "func.call @main_up_phase": 16,
        "func.call @main_swiglu_phase": 16,
        "func.call @main_down_phase": 16,
    }
    errors: list[str] = []
    ...
    if match is None:
        ...
        if runtime_args != [
            f"%weights: memref<{TOTAL_WEIGHT_DWORDS}xi32>",
    ...
    required_markers = (
        ...
        f"memref<{COLUMN_OUTPUT_DWORDS}xi32>",
        f"memref<{COLUMN_WEIGHT_DWORDS}xi32>",
        f"memref<{TOTAL_OUTPUT_DWORDS}xi32>",
        f"memref<{TOTAL_WEIGHT_DWORDS}xi32>",
        "aie.tile(2, 1)",
        "aie.tile(5, 5)",
        "aie.tile(0, 2)",
        "aie.tile(7, 5)",
    ...
    forbidden_runtime_names = ("%record", "%attention", "%edge", "%m0_", "%mt0_", "%gate", "%up", "%swiglu")
...
def run_on_npu() -> bool:
    print("=" * 72)
    print("Experiment 47: Main16 Multi-Phase FFN Replay")
    print("=" * 72)
    ...
    print(f"  tile weights: {TILE_WEIGHT_DWORDS} i32 = O + gate + up + down")
    print(f"  runtime weights/output: {TOTAL_WEIGHT_DWORDS}/{TOTAL_OUTPUT_DWORDS} i32")
    print()
    ...
    if mismatches.size == 0:
        print("  PASS: main16 multi-phase O/gate/up/down replay verified.")
        return True

    print(f"  FAIL: mismatches={mismatches.size}/{TOTAL_OUTPUT_DWORDS}")
