...
constexpr int GATE_WEIGHT_DWORDS = 32;
constexpr int UP_WEIGHT_DWORDS = 32;
constexpr int DOWN_WEIGHT_DWORDS = 32;
...
void main_gate_phase(int32_t *o_output, int32_t *weight, int32_t *gate, int32_t group, int32_t row) {
    static_assert(GATE_WEIGHT_DWORDS == SHARD_DWORDS);
    for (int lane = 0; lane < SHARD_DWORDS; lane++) {
        int32_t norm = o_output[lane] / 128 - ((group * 5 + row * 3 + lane) & 15) + 7;
        gate[lane] = norm * weight[lane] + weight[(lane + 11) & (SHARD_DWORDS - 1)] + group * 11 + row;
    }
}

void main_up_phase(int32_t *o_output, int32_t *weight, int32_t *up, int32_t group, int32_t row) {
    static_assert(UP_WEIGHT_DWORDS == SHARD_DWORDS);
    for (int lane = 0; lane < SHARD_DWORDS; lane++) {
        int32_t norm = o_output[lane] / 128 - ((group * 5 + row * 3 + lane) & 15) + 7;
        up[lane] = norm * weight[lane] - weight[(lane + 7) & (SHARD_DWORDS - 1)] + row * 17 + lane;
    }
}

void main_swiglu_phase(int32_t *gate, int32_t *up, int32_t *swiglu) {
    for (int lane = 0; lane < SHARD_DWORDS; lane++) {
        swiglu[lane] = (gate[lane] * up[lane]) / 64;
    }
}

void main_down_phase(int32_t *o_output, int32_t *swiglu, int32_t *weight, int32_t *output,
                     int32_t group, int32_t row) {
    static_assert(DOWN_WEIGHT_DWORDS == SHARD_DWORDS);
    for (int lane = 0; lane < SHARD_DWORDS; lane++) {
        output[lane] = o_output[lane] + swiglu[lane] -
                       ((swiglu[(lane + 9) & (SHARD_DWORDS - 1)] * weight[lane]) / 32) +
                       weight[(lane + 13) & (SHARD_DWORDS - 1)] +
