#!/usr/bin/env python3
"""Run exp27 on real NPU and verify the shape-A/shape-B attention contract."""
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    checks = {
        ...
        "aiex.npu.sync": 2,
        "aie.memtile_dma": 2,
        "func.call @shape_a_softmax_sideband": 1,
        "func.call @shape_b_weighted_value": 1,
    }
    errors: list[str] = []
...
def run_case(context_len: int, xclbin_path: Path, insts_path: Path) -> bool:
    print(f"\n--- L={context_len} valid={last_valid_for_context(context_len)} ---")
    kernel = NPUKernel(
...
def main() -> bool:
    print("=" * 72)
    print("Experiment 27: Shape-A/Shape-B Single-Tile Attention Contract")
    print("=" * 72)
    print(f"  current packet: {CURRENT_DWORDS} f32 via packet14")
    print(f"  K/V history: {HISTORY_SOURCE_DWORDS} f32 through row1 rings, first 2048 consumed")
    ...
    print("=" * 72)
    if all_passed:
        print("SUCCESS: exp27 shape-A/shape-B attention contract verified on NPU.")
    else:
        print("FAIL: at least one exp27 context length failed.")
    print("=" * 72)
    return all_passed
