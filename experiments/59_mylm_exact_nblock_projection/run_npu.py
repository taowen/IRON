#!/usr/bin/env python3
"""Run exp59 on real NPU and verify exact MyLM N-block projection."""
...
def run_on_npu() -> bool:
    print("=" * 78)
    print("Experiment 59: MyLM Exact N-Block Projection")
    print("=" * 78)
    ...
    print(f"  full N-block output rows: {len(MAIN_COLUMNS) * ROWS_PER_COLUMN * M_PER_TILE}")
    print()
    ...
    abs_tol = 0.5
    rel_tol = 0.18
    mismatch = np.where(abs_err > np.maximum(abs_tol, rel_tol * np.abs(expected_f32)))[0]
    ...
    if mismatch.size == 0:
        print("  PASS: exact MyLM 64-row patches fed a full 512-row main16 projection N-block.")
        return True

    print(f"  FAIL: mismatches={mismatch.size}/{TOTAL_OUTPUT_BF16 - len(MAIN_COLUMNS) * ROWS_PER_COLUMN * 2}")
