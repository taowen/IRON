"""Generate raw MLIR-AIE for exp59 exact MyLM N-block projection."""
...
{_lock_pair(tile, "wt", 0, init_empty=2)}
...
{_lock_pair(tile, "output", 4)}
    ...
    ^act_start:
      %1 = aie.dma_start(S2MM, 1, ^act_ping, ^out_start)
    ^act_ping:
      aie.use_lock(%{tile}_act_empty, AcquireGreaterEqual, 1)
    ...
    ^out_bd:
      ...
      aie.dma_bd(%{tile}_output : memref<{OUT_RECORD_BF16}xbf16>, 0, {OUT_RECORD_BF16}) {{bd_id = 2 : i32, packet = #aie.packet_info<pkt_type = 0, pkt_id = {_packet_id(group, row)}>}}
...
def _row_stream(group: int, row: int, patch: int, row_in_patch: int, bd_id: int) -> str:
    prefix = f"mt{group}"
    dims = (
        f"[<size = {K_CHUNKS}, stride = {2 * CHUNK_BF16}>, "
        f"<size = {CHUNK_BF16 // ACT_SLICE_BF16}, stride = {ACT_SLICE_BF16}>, "
        f"<size = {ACT_SLICE_BF16}, stride = 1>]"
    )
    ...
    ^row{row}_bd:
      ...
      aie.dma_bd(%{prefix}_patch{patch} : memref<{PATCH_BF16}xbf16>, {offset}, {K_CHUNKS * CHUNK_BF16}, {dims}) {{bd_id = {bd_id} : i32}}
...
def _memtile_column(group: int) -> str:
    ...
    row_bds = (
        _row_stream(group, 0, 0, 0, 2),
        _row_stream(group, 1, 0, 1, 24),
        _row_stream(group, 2, 1, 0, 4),
        _row_stream(group, 3, 1, 1, 26),
    )

    return f"""
    %{prefix}_patch0 = aie.buffer(%mt{group}) {{sym_name = "mt{group}_patch0"}} : memref<{PATCH_BF16}xbf16>
    %{prefix}_patch1 = aie.buffer(%mt{group}) {{sym_name = "mt{group}_patch1"}} : memref<{PATCH_BF16}xbf16>
    ...
    %{prefix}_patch1_empty = aie.lock(%mt{group}, 2) {{init = 2 : i32, sym_name = "mt{group}_patch1_empty"}}
    %{prefix}_patch1_full = aie.lock(%mt{group}, 3) {{init = 0 : i32, sym_name = "mt{group}_patch1_full"}}
    ...
    %mt{group}_dma = aie.memtile_dma(%mt{group}) {{
      %0 = aie.dma_start(S2MM, 0, ^patch0_in, ^row0_start)
    ^patch0_in:
      aie.use_lock(%{prefix}_patch0_empty, AcquireGreaterEqual, 2)
      aie.dma_bd(%{prefix}_patch0 : memref<{PATCH_BF16}xbf16>, 0, {PATCH_BF16}) {{bd_id = 0 : i32, next_bd_id = 1 : i32}}
      aie.use_lock(%{prefix}_patch0_full, Release, 2)
      aie.next_bd ^patch1_in
    ^patch1_in:
      aie.use_lock(%{prefix}_patch1_empty, AcquireGreaterEqual, 2)
      aie.dma_bd(%{prefix}_patch1 : memref<{PATCH_BF16}xbf16>, 0, {PATCH_BF16}) {{bd_id = 1 : i32, next_bd_id = 0 : i32}}
      aie.use_lock(%{prefix}_patch1_full, Release, 2)
      aie.next_bd ^patch0_in
...
{chr(10).join(output_bds)}
    ...
    ^drain:
      ...
      aie.dma_bd(%{prefix}_output : memref<{COLUMN_OUTPUT_BF16}xbf16>, 0, {COLUMN_OUTPUT_BF16}) {{bd_id = 34 : i32}}
...
def _runtime_sequence() -> str:
    ...
    for group, column in enumerate(MAIN_COLUMNS):
        ...
        lines += [
            _npu_writebd(column, 0, PATCH_BF16 // 2, column_base, next_bd=1, use_next_bd=True),
            _npu_address_patch(column, 0, 0, column_base),
            _npu_writebd(column, 1, PATCH_BF16 // 2, column_base + patch_bytes),
