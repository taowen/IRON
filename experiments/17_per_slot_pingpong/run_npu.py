#!/usr/bin/env python3
"""
Experiment 16: Scaled FFN — 4 cols × 4 rows, K=4096

Proves: 16-tile scale-out with BD double-buffer, multicast activation,
4-core packet gather, and production-scale Q4NX projections.
"""
...
def run_on_npu():
    ...
    print(f"  BD double-buffer: alternating bd1/bd2 for {PUSHES_PER_COL} pushes")
    ...
    print(f"  Ref output[128:132] (col1_r0): {ref_output[128:132]}")
    ...
    print(f"  NPU output[128:132] (col1_r0): {npu_output[128:132]}")
    ...
    # Tolerance: wider — two Q4NX projections at K=4096 + SwiGLU amplifies rounding
    rel_tol = 0.40
    abs_tol = 0.40
    ...
    print("=" * 70)
    if num_errors == 0:
        print("SUCCESS: 16-Tile Scaled FFN!")
        print(f"  {NUM_TILES} tiles ({NUM_COLS} cols × {ROWS_PER_COL} rows) = {TOTAL_OUTPUT} outputs verified")
        print(f"  K={K_HIDDEN}, {PUSHES_PER_COL} weight pushes/col via BD double-buffer")
...
if __name__ == "__main__":
    print("=" * 70)
    print("Experiment 16: 16-Tile Scaled FFN")
    print(f"  {NUM_COLS} cols × {ROWS_PER_COL} rows, K={K_HIDDEN}, BD double-buffer")
    print("=" * 70)
    print()
