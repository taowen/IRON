"""CPU reference for exp37 projected one-read fanout attention."""
...
HEAD_PHASE_BYTES = (HEAD_DIM // Q4_ROWS) * ROWBLOCK_BYTES
Q_PHASE_BYTES = NUM_Q_HEADS * HEAD_PHASE_BYTES
KV_PHASE_BYTES = HEAD_PHASE_BYTES
...
def project_qkv_reference(packed_weight: np.ndarray, hidden: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    k = _project_projection(packed_weight[k0:v0], hidden, HEAD_DIM)
    v = _project_projection(packed_weight[v0:v0 + KV_PHASE_BYTES], hidden, HEAD_DIM)
    return q, k, v
...
def make_case_data(seed: int = 42) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    k_params = _projection_params(rng, HEAD_DIM)
    v_params = _projection_params(rng, HEAD_DIM)
...
def apply_current_to_cache(
    ...
    out[k_offset:k_offset + HEAD_DIM] = current_k
    out[v_offset:v_offset + HEAD_DIM] = current_v
    return out
...
def online_attention_from_cache(context_len: int, query: np.ndarray, kv_cache: np.ndarray) -> np.ndarray:
    out = np.zeros(OUTPUT_DWORDS, dtype=np.float32)
...
def attention_reference_after_current_write(
    ...
    expected = online_attention_from_cache(context_len, query, cache_after)
    return query, current_k, current_v, cache_before, cache_after, expected


if __name__ == "__main__":
    hidden_data, weight_data, q, k, v = make_case_data()
    _, _, _, _, _, out = attention_reference_after_current_write(33, hidden_data, weight_data)
