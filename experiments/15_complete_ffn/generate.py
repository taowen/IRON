"""
Experiment 15: Complete FFN with Down Projection + Bidirectional Memtile Flow

Generates raw MLIR-AIE with:
- 2 columns × 2 rows = 4 compute tiles
- Cross-phase residency: gate/up outputs in core-local buffers (no DDR drain)
- SwiGLU kernel reads from local buffers
- Intermediate upload: cores → memtile (bidirectional flow)
- Memtile gather: concatenates 2 cores' intermediates
- Down projection: gathered intermediate → final output
- Sequential BD chains: S2MM ch0 (act→gathered), S2MM ch1 (4× wt → down_wt)
- All weights on single shim MM2S ch1 (gate/up + down, 5 pushes)

Core DMA constraint: 2 S2MM + 2 MM2S per core tile.
Shim DMA constraint: 2 MM2S + 2 S2MM per shim tile.
"""

from pathlib import Path

M_PER_TILE = 32
NUM_COLS = 2
ROWS_PER_COL = 2
K_HIDDEN = 512
K_CHUNK = 256
NUM_CHUNKS = K_HIDDEN // K_CHUNK  # 2
GROUP_SIZE = 32
NUM_PHASES_PROJ = 2  # gate + up

INTERMEDIATE = ROWS_PER_COL * M_PER_TILE  # 64
K_DOWN = INTERMEDIATE  # 64
K_CHUNK_DOWN = 64
NUM_CHUNKS_DOWN = 1
GROUPS_PER_ROW_DOWN = K_CHUNK_DOWN // GROUP_SIZE  # 2

CHUNK_BF16 = 2560
FAT_CHUNK_BF16 = CHUNK_BF16 * ROWS_PER_COL  # 5120
DOWN_CHUNK_BF16 = 640
FAT_DOWN_CHUNK_BF16 = DOWN_CHUNK_BF16 * ROWS_PER_COL  # 1280

ACT_BF16 = K_HIDDEN  # 512
OUT_BF16 = M_PER_TILE  # 32
INTER_BF16 = M_PER_TILE  # 32 (per core intermediate)
GATHERED_BF16 = INTERMEDIATE  # 64 (gathered from both cores)

TOTAL_OUTPUT = NUM_COLS * ROWS_PER_COL * M_PER_TILE  # 128

# Weight BO: gate/up fat chunks + down fat chunk, per column
PER_COL_WT_BF16 = NUM_PHASES_PROJ * NUM_CHUNKS * FAT_CHUNK_BF16 + FAT_DOWN_CHUNK_BF16
TOTAL_WT_I32 = (NUM_COLS * PER_COL_WT_BF16) // 2
ACT_I32 = K_HIDDEN // 2
OUT_TOTAL_I32 = TOTAL_OUTPUT // 2

# Number of weight pushes per column: 4 gate/up + 1 down = 5
NUM_WT_PUSHES = NUM_PHASES_PROJ * NUM_CHUNKS + 1


def _shim_bd_address(column: int, bd_id: int) -> int:
    return column * 0x02000000 + 0x1D004 + bd_id * 0x20


def _npu_writebd(column, bd_id, buffer_length, buffer_offset,
                 d0_size=0, d0_stride=0, d1_size=0, d1_stride=0,
                 d2_size=0, d2_stride=0,
                 iteration_size=0, iteration_stride=0):
    return (
        f"      aiex.npu.writebd {{bd_id = {bd_id} : i32, "
        f"buffer_length = {buffer_length} : i32, buffer_offset = {buffer_offset} : i32, "
        f"burst_length = 0 : i32, column = {column} : i32, "
        f"d0_size = {d0_size} : i32, d0_stride = {d0_stride} : i32, "
        f"d0_zero_after = 0 : i32, d0_zero_before = 0 : i32, "
        f"d1_size = {d1_size} : i32, d1_stride = {d1_stride} : i32, "
        f"d1_zero_after = 0 : i32, d1_zero_before = 0 : i32, "
        f"d2_size = {d2_size} : i32, d2_stride = {d2_stride} : i32, "
        f"d2_zero_after = 0 : i32, d2_zero_before = 0 : i32, "
        f"enable_packet = 0 : i32, iteration_current = 0 : i32, "
        f"iteration_size = {iteration_size} : i32, iteration_stride = {iteration_stride} : i32, "
        f"lock_acq_enable = 0 : i32, lock_acq_id = 0 : i32, lock_acq_val = 0 : i32, "
        f"lock_rel_id = 0 : i32, lock_rel_val = 0 : i32, "
        f"next_bd = 0 : i32, out_of_order_id = 0 : i32, "
        f"packet_id = 0 : i32, packet_type = 0 : i32, "
        f"row = 0 : i32, use_next_bd = 0 : i32, valid_bd = 1 : i32}}"
    )


def _npu_address_patch(column, bd_id, arg_idx, arg_plus_bytes):
    addr = _shim_bd_address(column, bd_id)
    return f"      aiex.npu.address_patch {{addr = {addr} : ui32, arg_idx = {arg_idx} : i32, arg_plus = {arg_plus_bytes} : i32}}"


def _npu_push_queue(column, direction, channel, bd_id, issue_token=False, repeat_count=0):
    token = "true" if issue_token else "false"
    return f"      aiex.npu.push_queue({column}, 0, {direction} : {channel}) {{bd_id = {bd_id} : i32, issue_token = {token}, repeat_count = {repeat_count} : i32}}"


def _npu_sync(column, channel=0):
    return f"      aiex.npu.sync {{channel = {channel} : i32, column = {column} : i32, column_num = 1 : i32, direction = 0 : i32, row = 0 : i32, row_num = 1 : i32}}"


def _core_program(col, row, prefix):
    """Generate core program: gate → up → swiglu → intermediate upload → down projection.

    DMA channels used:
      S2MM ch0: activation (BD0) → gathered intermediate (BD6) [sequential]
      S2MM ch1: 4× gate/up weight (BD1-4) → 1× down weight (BD5) [sequential 5-step]
      MM2S ch0: final output (BD7)
      MM2S ch1: intermediate upload (BD8)
    """
    return f"""    %core_{prefix} = aie.core(%{prefix}) {{
      %c0 = arith.constant 0 : index
      %c1 = arith.constant 1 : index
      %c2_i32 = arith.constant 2 : i32
      %c1_i32 = arith.constant 1 : i32
      %c0_i32 = arith.constant 0 : i32
      %num_chunks = arith.constant {NUM_CHUNKS} : index
      %k_chunk_i32 = arith.constant {K_CHUNK} : i32
      %m_i32 = arith.constant {M_PER_TILE} : i32

      // Acquire activation — hold across gate and up phases
      aie.use_lock(%{prefix}_act_full, AcquireGreaterEqual, 1)

      // Phase 1: GATE projection — {NUM_CHUNKS} chunks, flush to LOCAL gate_buf
      scf.for %chunk = %c0 to %num_chunks step %c1 {{
        %chunk_i32 = arith.index_cast %chunk : index to i32
        %act_offset = arith.muli %chunk_i32, %k_chunk_i32 : i32
        %rem = arith.remsi %chunk_i32, %c2_i32 : i32
        %is_pong = arith.cmpi eq, %rem, %c1_i32 : i32

        aie.use_lock(%{prefix}_wt_full, AcquireGreaterEqual, 1)

        scf.if %is_pong {{
          func.call @q4nx_chunk_accum_offset(%{prefix}_wt_pong, %{prefix}_act, %act_offset, %m_i32)
            : (memref<{CHUNK_BF16}xbf16>, memref<{ACT_BF16}xbf16>, i32, i32) -> ()
        }} else {{
          func.call @q4nx_chunk_accum_offset(%{prefix}_wt_ping, %{prefix}_act, %act_offset, %m_i32)
            : (memref<{CHUNK_BF16}xbf16>, memref<{ACT_BF16}xbf16>, i32, i32) -> ()
        }}

        aie.use_lock(%{prefix}_wt_empty, Release, 1)
      }}
      func.call @q4nx_flush_output(%{prefix}_gate_buf, %m_i32)
        : (memref<{OUT_BF16}xbf16>, i32) -> ()

      // Phase 2: UP projection — {NUM_CHUNKS} chunks, flush to LOCAL up_buf
      scf.for %chunk = %c0 to %num_chunks step %c1 {{
        %chunk_i32 = arith.index_cast %chunk : index to i32
        %act_offset = arith.muli %chunk_i32, %k_chunk_i32 : i32
        %rem = arith.remsi %chunk_i32, %c2_i32 : i32
        %is_pong = arith.cmpi eq, %rem, %c1_i32 : i32

        aie.use_lock(%{prefix}_wt_full, AcquireGreaterEqual, 1)

        scf.if %is_pong {{
          func.call @q4nx_chunk_accum_offset(%{prefix}_wt_pong, %{prefix}_act, %act_offset, %m_i32)
            : (memref<{CHUNK_BF16}xbf16>, memref<{ACT_BF16}xbf16>, i32, i32) -> ()
        }} else {{
          func.call @q4nx_chunk_accum_offset(%{prefix}_wt_ping, %{prefix}_act, %act_offset, %m_i32)
            : (memref<{CHUNK_BF16}xbf16>, memref<{ACT_BF16}xbf16>, i32, i32) -> ()
        }}

        aie.use_lock(%{prefix}_wt_empty, Release, 1)
      }}
      func.call @q4nx_flush_output(%{prefix}_up_buf, %m_i32)
        : (memref<{OUT_BF16}xbf16>, i32) -> ()

      // Phase 3: SwiGLU — writes to inter_buf (stays in fabric)
      func.call @swiglu_fused(%{prefix}_gate_buf, %{prefix}_up_buf, %{prefix}_inter_buf, %m_i32)
        : (memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, memref<{INTER_BF16}xbf16>, i32) -> ()

      // Release activation (no longer needed)
      aie.use_lock(%{prefix}_act_empty, Release, 1)

      // Send intermediate to memtile via packet-switch
      aie.use_lock(%{prefix}_inter_cons, Release, 1)

      // Phase 4: DOWN projection — wait for gathered + down weight
      aie.use_lock(%{prefix}_gathered_full, AcquireGreaterEqual, 1)
      aie.use_lock(%{prefix}_down_wt_full, AcquireGreaterEqual, 1)

      func.call @q4nx_down_chunk_accum_offset(%{prefix}_down_wt_buf, %{prefix}_gathered_buf, %c0_i32, %m_i32)
        : (memref<{DOWN_CHUNK_BF16}xbf16>, memref<{GATHERED_BF16}xbf16>, i32, i32) -> ()
      aie.use_lock(%{prefix}_down_wt_empty, Release, 1)

      // Flush down projection output
      aie.use_lock(%{prefix}_out_prod, AcquireGreaterEqual, 1)
      func.call @q4nx_down_flush_output(%{prefix}_out, %m_i32)
        : (memref<{OUT_BF16}xbf16>, i32) -> ()
      aie.use_lock(%{prefix}_out_cons, Release, 1)

      // Release gathered buffer
      aie.use_lock(%{prefix}_gathered_empty, Release, 1)

      aie.end
    }}"""


def _memtile_dma(col, prefix):
    """Generate memtile DMA with bidirectional flow.

    S2MM channels:
      ch0 (even, BD 0): activation from shim — 2-consumer
      ch1 (odd, BD 24-28): all weights from shim — 4× gate/up fat + 1× down fat [5-step sequential]
      ch3 (odd, BD 29→9): BOTH intermediates from cores via packet-switch [sequential BDs]

    MM2S channels:
      ch0 (even, BD 2→3): act to core0, THEN gathered to core0 (sequential)
      ch3 (odd, BD 30→31): act to core1, THEN gathered to core1 (sequential)
      ch1 (odd, BD 32-36): wt first half (4×) + down first half (1×) → core0 [5-step]
      ch2 (even, BD 4-8): wt second half (4×) + down second half (1×) → core1 [5-step]
    """
    return f"""    %memtile_dma_{prefix} = aie.memtile_dma(%mt{col}) {{
      // S2MM ch0 (even → BD 0-23): activation from shim (2-consumer)
      %0 = aie.dma_start(S2MM, 0, ^act_s2mm, ^inter_s2mm_start)
    ^act_s2mm:
      aie.use_lock(%mt{col}_act_empty, AcquireGreaterEqual, 2)
      aie.dma_bd(%mt{col}_act_buf : memref<{ACT_BF16}xbf16>, 0, {ACT_BF16}) {{bd_id = 0 : i32, next_bd_id = 0 : i32}}
      aie.use_lock(%mt{col}_act_full, Release, 2)
      aie.next_bd ^act_s2mm

      // S2MM ch3 (odd → BD 24-47): BOTH intermediates via packet-switch, sequential BDs
    ^inter_s2mm_start:
      %1 = aie.dma_start(S2MM, 3, ^inter_s2mm_bd0, ^act_mm2s_r0_start)
    ^inter_s2mm_bd0:
      aie.use_lock(%mt{col}_gathered_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_gathered_buf : memref<{GATHERED_BF16}xbf16>, 0, {INTER_BF16}) {{bd_id = 29 : i32, next_bd_id = 37 : i32}}
      aie.use_lock(%mt{col}_gathered_full, Release, 1)
      aie.next_bd ^inter_s2mm_bd1
    ^inter_s2mm_bd1:
      aie.use_lock(%mt{col}_gathered_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_gathered_buf : memref<{GATHERED_BF16}xbf16>, {INTER_BF16}, {INTER_BF16}) {{bd_id = 37 : i32, next_bd_id = 29 : i32}}
      aie.use_lock(%mt{col}_gathered_full, Release, 1)
      aie.next_bd ^inter_s2mm_bd0

      // MM2S ch0 (even → BD 0-23): act to core0 THEN gathered to core0 (sequential)
    ^act_mm2s_r0_start:
      %2 = aie.dma_start(MM2S, 0, ^act_mm2s_r0, ^wt_mm2s_r1_start)
    ^act_mm2s_r0:
      aie.use_lock(%mt{col}_act_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_act_buf : memref<{ACT_BF16}xbf16>, 0, {ACT_BF16}) {{bd_id = 2 : i32, next_bd_id = 3 : i32}}
      aie.use_lock(%mt{col}_act_empty, Release, 1)
      aie.next_bd ^gathered_mm2s_r0
    ^gathered_mm2s_r0:
      aie.use_lock(%mt{col}_gathered_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_gathered_buf : memref<{GATHERED_BF16}xbf16>, 0, {GATHERED_BF16}) {{bd_id = 3 : i32, next_bd_id = 2 : i32}}
      aie.use_lock(%mt{col}_gathered_empty, Release, 1)
      aie.next_bd ^act_mm2s_r0

      // MM2S ch2 (even → BD 0-23): wt second half (4×) + down second half (1×) → core1
    ^wt_mm2s_r1_start:
      %3 = aie.dma_start(MM2S, 2, ^wt_mm2s_r1_a, ^wt_s2mm_ch1_start)
    ^wt_mm2s_r1_a:
      aie.use_lock(%mt{col}_wt_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_wt_ping : memref<{FAT_CHUNK_BF16}xbf16>, {CHUNK_BF16}, {CHUNK_BF16}) {{bd_id = 4 : i32, next_bd_id = 5 : i32}}
      aie.use_lock(%mt{col}_wt_empty, Release, 1)
      aie.next_bd ^wt_mm2s_r1_b
    ^wt_mm2s_r1_b:
      aie.use_lock(%mt{col}_wt_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_wt_pong : memref<{FAT_CHUNK_BF16}xbf16>, {CHUNK_BF16}, {CHUNK_BF16}) {{bd_id = 5 : i32, next_bd_id = 6 : i32}}
      aie.use_lock(%mt{col}_wt_empty, Release, 1)
      aie.next_bd ^wt_mm2s_r1_c
    ^wt_mm2s_r1_c:
      aie.use_lock(%mt{col}_wt_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_wt_ping : memref<{FAT_CHUNK_BF16}xbf16>, {CHUNK_BF16}, {CHUNK_BF16}) {{bd_id = 6 : i32, next_bd_id = 7 : i32}}
      aie.use_lock(%mt{col}_wt_empty, Release, 1)
      aie.next_bd ^wt_mm2s_r1_d
    ^wt_mm2s_r1_d:
      aie.use_lock(%mt{col}_wt_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_wt_pong : memref<{FAT_CHUNK_BF16}xbf16>, {CHUNK_BF16}, {CHUNK_BF16}) {{bd_id = 7 : i32, next_bd_id = 8 : i32}}
      aie.use_lock(%mt{col}_wt_empty, Release, 1)
      aie.next_bd ^down_mm2s_r1
    ^down_mm2s_r1:
      aie.use_lock(%mt{col}_down_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_down_buf : memref<{FAT_DOWN_CHUNK_BF16}xbf16>, {DOWN_CHUNK_BF16}, {DOWN_CHUNK_BF16}) {{bd_id = 8 : i32, next_bd_id = 4 : i32}}
      aie.use_lock(%mt{col}_down_empty, Release, 1)
      aie.next_bd ^wt_mm2s_r1_a

      // S2MM ch1 (odd → BD 24-47): all weights from shim — 4 fat + 1 down [5-step]
    ^wt_s2mm_ch1_start:
      %4 = aie.dma_start(S2MM, 1, ^wt_s2mm_a, ^act_mm2s_r1_start)
    ^wt_s2mm_a:
      aie.use_lock(%mt{col}_wt_empty, AcquireGreaterEqual, 2)
      aie.dma_bd(%mt{col}_wt_ping : memref<{FAT_CHUNK_BF16}xbf16>, 0, {FAT_CHUNK_BF16}) {{bd_id = 24 : i32, next_bd_id = 25 : i32}}
      aie.use_lock(%mt{col}_wt_full, Release, 2)
      aie.next_bd ^wt_s2mm_b
    ^wt_s2mm_b:
      aie.use_lock(%mt{col}_wt_empty, AcquireGreaterEqual, 2)
      aie.dma_bd(%mt{col}_wt_pong : memref<{FAT_CHUNK_BF16}xbf16>, 0, {FAT_CHUNK_BF16}) {{bd_id = 25 : i32, next_bd_id = 26 : i32}}
      aie.use_lock(%mt{col}_wt_full, Release, 2)
      aie.next_bd ^wt_s2mm_c
    ^wt_s2mm_c:
      aie.use_lock(%mt{col}_wt_empty, AcquireGreaterEqual, 2)
      aie.dma_bd(%mt{col}_wt_ping : memref<{FAT_CHUNK_BF16}xbf16>, 0, {FAT_CHUNK_BF16}) {{bd_id = 26 : i32, next_bd_id = 27 : i32}}
      aie.use_lock(%mt{col}_wt_full, Release, 2)
      aie.next_bd ^wt_s2mm_d
    ^wt_s2mm_d:
      aie.use_lock(%mt{col}_wt_empty, AcquireGreaterEqual, 2)
      aie.dma_bd(%mt{col}_wt_pong : memref<{FAT_CHUNK_BF16}xbf16>, 0, {FAT_CHUNK_BF16}) {{bd_id = 27 : i32, next_bd_id = 28 : i32}}
      aie.use_lock(%mt{col}_wt_full, Release, 2)
      aie.next_bd ^down_s2mm
    ^down_s2mm:
      aie.use_lock(%mt{col}_down_empty, AcquireGreaterEqual, 2)
      aie.dma_bd(%mt{col}_down_buf : memref<{FAT_DOWN_CHUNK_BF16}xbf16>, 0, {FAT_DOWN_CHUNK_BF16}) {{bd_id = 28 : i32, next_bd_id = 24 : i32}}
      aie.use_lock(%mt{col}_down_full, Release, 2)
      aie.next_bd ^wt_s2mm_a

      // MM2S ch3 (odd → BD 24-47): act to core1 THEN gathered to core1 (sequential)
    ^act_mm2s_r1_start:
      %5 = aie.dma_start(MM2S, 3, ^act_mm2s_r1, ^wt_mm2s_r0_start)
    ^act_mm2s_r1:
      aie.use_lock(%mt{col}_act_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_act_buf : memref<{ACT_BF16}xbf16>, 0, {ACT_BF16}) {{bd_id = 30 : i32, next_bd_id = 31 : i32}}
      aie.use_lock(%mt{col}_act_empty, Release, 1)
      aie.next_bd ^gathered_mm2s_r1
    ^gathered_mm2s_r1:
      aie.use_lock(%mt{col}_gathered_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_gathered_buf : memref<{GATHERED_BF16}xbf16>, 0, {GATHERED_BF16}) {{bd_id = 31 : i32, next_bd_id = 30 : i32}}
      aie.use_lock(%mt{col}_gathered_empty, Release, 1)
      aie.next_bd ^act_mm2s_r1

      // MM2S ch1 (odd → BD 24-47): wt first half (4×) + down first half (1×) → core0
    ^wt_mm2s_r0_start:
      %6 = aie.dma_start(MM2S, 1, ^wt_mm2s_r0_a, ^end)
    ^wt_mm2s_r0_a:
      aie.use_lock(%mt{col}_wt_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_wt_ping : memref<{FAT_CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 32 : i32, next_bd_id = 33 : i32}}
      aie.use_lock(%mt{col}_wt_empty, Release, 1)
      aie.next_bd ^wt_mm2s_r0_b
    ^wt_mm2s_r0_b:
      aie.use_lock(%mt{col}_wt_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_wt_pong : memref<{FAT_CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 33 : i32, next_bd_id = 34 : i32}}
      aie.use_lock(%mt{col}_wt_empty, Release, 1)
      aie.next_bd ^wt_mm2s_r0_c
    ^wt_mm2s_r0_c:
      aie.use_lock(%mt{col}_wt_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_wt_ping : memref<{FAT_CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 34 : i32, next_bd_id = 35 : i32}}
      aie.use_lock(%mt{col}_wt_empty, Release, 1)
      aie.next_bd ^wt_mm2s_r0_d
    ^wt_mm2s_r0_d:
      aie.use_lock(%mt{col}_wt_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_wt_pong : memref<{FAT_CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 35 : i32, next_bd_id = 36 : i32}}
      aie.use_lock(%mt{col}_wt_empty, Release, 1)
      aie.next_bd ^down_mm2s_r0
    ^down_mm2s_r0:
      aie.use_lock(%mt{col}_down_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%mt{col}_down_buf : memref<{FAT_DOWN_CHUNK_BF16}xbf16>, 0, {DOWN_CHUNK_BF16}) {{bd_id = 36 : i32, next_bd_id = 32 : i32}}
      aie.use_lock(%mt{col}_down_empty, Release, 1)
      aie.next_bd ^wt_mm2s_r0_a
    ^end:
      aie.end
    }}"""


def _core_mem(prefix, col, row):
    """Generate core tile DMA.

    S2MM ch0: activation (BD0) → gathered (BD6) [sequential chain]
    S2MM ch1: wt_ping(BD1) → wt_pong(BD2) → wt_ping(BD3) → wt_pong(BD4) → down_wt(BD5) [5-step]
    MM2S ch0: final output (BD7)
    MM2S ch1: intermediate upload (BD8) with packet header
    """
    return f"""    %mem_{prefix} = aie.mem(%{prefix}) {{
      // S2MM ch0: activation → gathered (sequential)
      %0 = aie.dma_start(S2MM, 0, ^act_bd, ^wt_start)
    ^act_bd:
      aie.use_lock(%{prefix}_act_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_act : memref<{ACT_BF16}xbf16>, 0, {ACT_BF16}) {{bd_id = 0 : i32, next_bd_id = 6 : i32}}
      aie.use_lock(%{prefix}_act_full, Release, 1)
      aie.next_bd ^gathered_bd
    ^gathered_bd:
      aie.use_lock(%{prefix}_gathered_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_gathered_buf : memref<{GATHERED_BF16}xbf16>, 0, {GATHERED_BF16}) {{bd_id = 6 : i32, next_bd_id = 0 : i32}}
      aie.use_lock(%{prefix}_gathered_full, Release, 1)
      aie.next_bd ^act_bd

      // S2MM ch1: 4× gate/up weight + 1× down weight [5-step sequential]
    ^wt_start:
      %1 = aie.dma_start(S2MM, 1, ^wt_bd0, ^out_start)
    ^wt_bd0:
      aie.use_lock(%{prefix}_wt_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_wt_ping : memref<{CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 1 : i32, next_bd_id = 2 : i32}}
      aie.use_lock(%{prefix}_wt_full, Release, 1)
      aie.next_bd ^wt_bd1
    ^wt_bd1:
      aie.use_lock(%{prefix}_wt_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_wt_pong : memref<{CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 2 : i32, next_bd_id = 3 : i32}}
      aie.use_lock(%{prefix}_wt_full, Release, 1)
      aie.next_bd ^wt_bd2
    ^wt_bd2:
      aie.use_lock(%{prefix}_wt_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_wt_ping : memref<{CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 3 : i32, next_bd_id = 4 : i32}}
      aie.use_lock(%{prefix}_wt_full, Release, 1)
      aie.next_bd ^wt_bd3
    ^wt_bd3:
      aie.use_lock(%{prefix}_wt_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_wt_pong : memref<{CHUNK_BF16}xbf16>, 0, {CHUNK_BF16}) {{bd_id = 4 : i32, next_bd_id = 5 : i32}}
      aie.use_lock(%{prefix}_wt_full, Release, 1)
      aie.next_bd ^down_wt_bd
    ^down_wt_bd:
      aie.use_lock(%{prefix}_down_wt_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_down_wt_buf : memref<{DOWN_CHUNK_BF16}xbf16>, 0, {DOWN_CHUNK_BF16}) {{bd_id = 5 : i32, next_bd_id = 1 : i32}}
      aie.use_lock(%{prefix}_down_wt_full, Release, 1)
      aie.next_bd ^wt_bd0

      // MM2S ch0: final output
    ^out_start:
      %2 = aie.dma_start(MM2S, 0, ^out_bd, ^inter_start)
    ^out_bd:
      aie.use_lock(%{prefix}_out_cons, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_out : memref<{OUT_BF16}xbf16>, 0, {OUT_BF16}) {{bd_id = 7 : i32, next_bd_id = 7 : i32}}
      aie.use_lock(%{prefix}_out_prod, Release, 1)
      aie.next_bd ^out_bd

      // MM2S ch1: intermediate upload to memtile (packet-switched)
    ^inter_start:
      %3 = aie.dma_start(MM2S, 1, ^inter_bd, ^end)
    ^inter_bd:
      aie.use_lock(%{prefix}_inter_cons, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_inter_buf : memref<{INTER_BF16}xbf16>, 0, {INTER_BF16}) {{bd_id = 8 : i32, next_bd_id = 8 : i32, packet = #aie.packet_info<pkt_type = 0, pkt_id = {col*2 + row}>}}
      aie.use_lock(%{prefix}_inter_prod, Release, 1)
      aie.next_bd ^inter_bd
    ^end:
      aie.end
    }}"""


def generate_mlir() -> str:
    experiment_dir = Path(__file__).parent.resolve()

    # Runtime sequence
    rt_lines = []
    rt_lines.append(f"    aie.runtime_sequence(%wt_bo: memref<{TOTAL_WT_I32}xi32>, %act_bo: memref<{ACT_I32}xi32>, %out_bo: memref<{OUT_TOTAL_I32}xi32>) {{")

    per_col_wt_bytes = PER_COL_WT_BF16 * 2
    fat_chunk_bytes = FAT_CHUNK_BF16 * 2  # 10240
    fat_down_bytes = FAT_DOWN_CHUNK_BF16 * 2  # 2560

    for col in range(NUM_COLS):
        # Activation: shim MM2S ch0, bd_id=0
        rt_lines.append(_npu_writebd(col, 0, ACT_I32, 0))
        rt_lines.append(_npu_address_patch(col, 0, 1, 0))
        rt_lines.append(_npu_push_queue(col, "MM2S", 0, 0))

        # All weights on shim MM2S ch1: 4 gate/up fat chunks + 1 down fat chunk
        base_offset = col * per_col_wt_bytes
        for i in range(NUM_PHASES_PROJ * NUM_CHUNKS):
            bd_id = 1 + i  # BD IDs 1-4
            offset = base_offset + i * fat_chunk_bytes
            rt_lines.append(_npu_writebd(col, bd_id, FAT_CHUNK_BF16 // 2, offset))
            rt_lines.append(_npu_address_patch(col, bd_id, 0, offset))
            rt_lines.append(_npu_push_queue(col, "MM2S", 1, bd_id))

        # Down weight: also on MM2S ch1 (BD ID 5)
        down_offset = base_offset + NUM_PHASES_PROJ * NUM_CHUNKS * fat_chunk_bytes
        rt_lines.append(_npu_writebd(col, 5, FAT_DOWN_CHUNK_BF16 // 2, down_offset))
        rt_lines.append(_npu_address_patch(col, 5, 0, down_offset))
        rt_lines.append(_npu_push_queue(col, "MM2S", 1, 5))

        # Output: S2MM ch0 for row0, S2MM ch1 for row1 (issue token for ALL)
        out_off_r0 = (col * ROWS_PER_COL + 0) * OUT_BF16 * 2
        rt_lines.append(_npu_writebd(col, 6, OUT_BF16 // 2, out_off_r0))
        rt_lines.append(_npu_address_patch(col, 6, 2, out_off_r0))
        rt_lines.append(_npu_push_queue(col, "S2MM", 0, 6, issue_token=True))

        out_off_r1 = (col * ROWS_PER_COL + 1) * OUT_BF16 * 2
        rt_lines.append(_npu_writebd(col, 7, OUT_BF16 // 2, out_off_r1))
        rt_lines.append(_npu_address_patch(col, 7, 2, out_off_r1))
        rt_lines.append(_npu_push_queue(col, "S2MM", 1, 7, issue_token=True))

    # Sync on ALL columns (wait for every output)
    for col in range(NUM_COLS):
        rt_lines.append(_npu_sync(col, 0))
        rt_lines.append(_npu_sync(col, 1))
    rt_lines.append("    }")
    runtime_sequence = "\n".join(rt_lines)

    # Tile/buffer/lock/flow declarations
    tiles = ""
    buffers = ""
    locks = ""
    flows = ""

    for col in range(NUM_COLS):
        tiles += f"    %shim{col} = aie.tile({col}, 0)\n"
        tiles += f"    %mt{col} = aie.tile({col}, 1)\n"
        for row in range(ROWS_PER_COL):
            r = row + 2
            prefix = f"c{col}r{row}"
            tiles += f"    %{prefix} = aie.tile({col}, {r})\n"

    tiles += "\n"

    for col in range(NUM_COLS):
        # Memtile buffers
        buffers += f"    %mt{col}_act_buf = aie.buffer(%mt{col}) {{sym_name = \"mt{col}_act_buf\"}} : memref<{ACT_BF16}xbf16>\n"
        buffers += f"    %mt{col}_wt_ping = aie.buffer(%mt{col}) {{sym_name = \"mt{col}_wt_ping\"}} : memref<{FAT_CHUNK_BF16}xbf16>\n"
        buffers += f"    %mt{col}_wt_pong = aie.buffer(%mt{col}) {{sym_name = \"mt{col}_wt_pong\"}} : memref<{FAT_CHUNK_BF16}xbf16>\n"
        buffers += f"    %mt{col}_gathered_buf = aie.buffer(%mt{col}) {{sym_name = \"mt{col}_gathered_buf\"}} : memref<{GATHERED_BF16}xbf16>\n"
        buffers += f"    %mt{col}_down_buf = aie.buffer(%mt{col}) {{sym_name = \"mt{col}_down_buf\"}} : memref<{FAT_DOWN_CHUNK_BF16}xbf16>\n"

        # Memtile locks
        locks += f"    %mt{col}_act_empty = aie.lock(%mt{col}, 0) {{init = 2 : i32, sym_name = \"mt{col}_act_empty\"}}\n"
        locks += f"    %mt{col}_act_full  = aie.lock(%mt{col}, 1) {{init = 0 : i32, sym_name = \"mt{col}_act_full\"}}\n"
        locks += f"    %mt{col}_wt_empty  = aie.lock(%mt{col}, 2) {{init = 4 : i32, sym_name = \"mt{col}_wt_empty\"}}\n"
        locks += f"    %mt{col}_wt_full   = aie.lock(%mt{col}, 3) {{init = 0 : i32, sym_name = \"mt{col}_wt_full\"}}\n"
        locks += f"    %mt{col}_gathered_empty = aie.lock(%mt{col}, 4) {{init = 2 : i32, sym_name = \"mt{col}_gathered_empty\"}}\n"
        locks += f"    %mt{col}_gathered_full  = aie.lock(%mt{col}, 5) {{init = 0 : i32, sym_name = \"mt{col}_gathered_full\"}}\n"
        locks += f"    %mt{col}_gathered_done  = aie.lock(%mt{col}, 8) {{init = 0 : i32, sym_name = \"mt{col}_gathered_done\"}}\n"
        locks += f"    %mt{col}_down_empty = aie.lock(%mt{col}, 6) {{init = 2 : i32, sym_name = \"mt{col}_down_empty\"}}\n"
        locks += f"    %mt{col}_down_full  = aie.lock(%mt{col}, 7) {{init = 0 : i32, sym_name = \"mt{col}_down_full\"}}\n"

        for row in range(ROWS_PER_COL):
            prefix = f"c{col}r{row}"
            # Core buffers
            buffers += f"    %{prefix}_act = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_act\"}} : memref<{ACT_BF16}xbf16>\n"
            buffers += f"    %{prefix}_wt_ping = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_wt_ping\"}} : memref<{CHUNK_BF16}xbf16>\n"
            buffers += f"    %{prefix}_wt_pong = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_wt_pong\"}} : memref<{CHUNK_BF16}xbf16>\n"
            buffers += f"    %{prefix}_out = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_out\"}} : memref<{OUT_BF16}xbf16>\n"
            # Core-LOCAL buffers (cross-phase residency)
            buffers += f"    %{prefix}_gate_buf = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_gate_buf\"}} : memref<{OUT_BF16}xbf16>\n"
            buffers += f"    %{prefix}_up_buf = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_up_buf\"}} : memref<{OUT_BF16}xbf16>\n"
            # New for exp 15: intermediate, gathered, down weight
            buffers += f"    %{prefix}_inter_buf = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_inter_buf\"}} : memref<{INTER_BF16}xbf16>\n"
            buffers += f"    %{prefix}_gathered_buf = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_gathered_buf\"}} : memref<{GATHERED_BF16}xbf16>\n"
            buffers += f"    %{prefix}_down_wt_buf = aie.buffer(%{prefix}) {{sym_name = \"{prefix}_down_wt_buf\"}} : memref<{DOWN_CHUNK_BF16}xbf16>\n"

            # Core locks
            locks += f"    %{prefix}_act_empty = aie.lock(%{prefix}, 0) {{init = 1 : i32, sym_name = \"{prefix}_act_empty\"}}\n"
            locks += f"    %{prefix}_act_full  = aie.lock(%{prefix}, 1) {{init = 0 : i32, sym_name = \"{prefix}_act_full\"}}\n"
            locks += f"    %{prefix}_wt_empty  = aie.lock(%{prefix}, 2) {{init = 2 : i32, sym_name = \"{prefix}_wt_empty\"}}\n"
            locks += f"    %{prefix}_wt_full   = aie.lock(%{prefix}, 3) {{init = 0 : i32, sym_name = \"{prefix}_wt_full\"}}\n"
            locks += f"    %{prefix}_out_prod  = aie.lock(%{prefix}, 4) {{init = 1 : i32, sym_name = \"{prefix}_out_prod\"}}\n"
            locks += f"    %{prefix}_out_cons  = aie.lock(%{prefix}, 5) {{init = 0 : i32, sym_name = \"{prefix}_out_cons\"}}\n"
            locks += f"    %{prefix}_inter_prod  = aie.lock(%{prefix}, 6) {{init = 0 : i32, sym_name = \"{prefix}_inter_prod\"}}\n"
            locks += f"    %{prefix}_inter_cons  = aie.lock(%{prefix}, 7) {{init = 0 : i32, sym_name = \"{prefix}_inter_cons\"}}\n"
            locks += f"    %{prefix}_gathered_empty = aie.lock(%{prefix}, 8) {{init = 1 : i32, sym_name = \"{prefix}_gathered_empty\"}}\n"
            locks += f"    %{prefix}_gathered_full  = aie.lock(%{prefix}, 9) {{init = 0 : i32, sym_name = \"{prefix}_gathered_full\"}}\n"
            locks += f"    %{prefix}_down_wt_empty = aie.lock(%{prefix}, 10) {{init = 1 : i32, sym_name = \"{prefix}_down_wt_empty\"}}\n"
            locks += f"    %{prefix}_down_wt_full  = aie.lock(%{prefix}, 11) {{init = 0 : i32, sym_name = \"{prefix}_down_wt_full\"}}\n"

    for col in range(NUM_COLS):
        # Shim → Memtile (2 flows)
        flows += f"    aie.flow(%shim{col}, DMA : 0, %mt{col}, DMA : 0)\n"  # activation
        flows += f"    aie.flow(%shim{col}, DMA : 1, %mt{col}, DMA : 1)\n"  # all weights

        # Memtile → Core (4 flows)
        flows += f"    aie.flow(%mt{col}, DMA : 0, %c{col}r0, DMA : 0)\n"  # act + gathered → core0
        flows += f"    aie.flow(%mt{col}, DMA : 3, %c{col}r1, DMA : 0)\n"  # act + gathered → core1
        flows += f"    aie.flow(%mt{col}, DMA : 1, %c{col}r0, DMA : 1)\n"  # wt + down_wt → core0
        flows += f"    aie.flow(%mt{col}, DMA : 2, %c{col}r1, DMA : 1)\n"  # wt + down_wt → core1

        # Core → Memtile (packet-switched to SAME S2MM ch3, sequential BDs)
        flows += f"    aie.packet_flow({col*2}) {{\n"
        flows += f"      aie.packet_source<%c{col}r0, DMA : 1>\n"
        flows += f"      aie.packet_dest<%mt{col}, DMA : 3>\n"
        flows += f"    }}\n"
        flows += f"    aie.packet_flow({col*2+1}) {{\n"
        flows += f"      aie.packet_source<%c{col}r1, DMA : 1>\n"
        flows += f"      aie.packet_dest<%mt{col}, DMA : 3>\n"
        flows += f"    }}\n"

        # Core → Shim (2 flows, final output)
        flows += f"    aie.flow(%c{col}r0, DMA : 0, %shim{col}, DMA : 0)\n"
        flows += f"    aie.flow(%c{col}r1, DMA : 0, %shim{col}, DMA : 1)\n"

    # Core programs
    core_programs = ""
    for col in range(NUM_COLS):
        for row in range(ROWS_PER_COL):
            prefix = f"c{col}r{row}"
            core_programs += _core_program(col, row, prefix) + "\n\n"

    # Memtile DMAs
    memtile_dmas = ""
    for col in range(NUM_COLS):
        prefix = f"mt{col}"
        memtile_dmas += _memtile_dma(col, prefix) + "\n\n"

    # Core MEMs
    core_mems = ""
    for col in range(NUM_COLS):
        for row in range(ROWS_PER_COL):
            prefix = f"c{col}r{row}"
            core_mems += _core_mem(prefix, col, row) + "\n\n"

    return f"""module {{
  aie.device(npu2) {{
    // === Tile declarations ===
{tiles}
    // === Buffers ===
{buffers}
    // === Locks ===
{locks}
    // === Flows ===
{flows}
    // === Kernel declarations ===
    func.func private @q4nx_chunk_accum_offset(memref<{CHUNK_BF16}xbf16>, memref<{ACT_BF16}xbf16>, i32, i32) attributes {{link_with = "{experiment_dir}/q4nx_chunk_accum.o"}}
    func.func private @q4nx_flush_output(memref<{OUT_BF16}xbf16>, i32) attributes {{link_with = "{experiment_dir}/q4nx_chunk_accum.o"}}
    func.func private @q4nx_down_chunk_accum_offset(memref<{DOWN_CHUNK_BF16}xbf16>, memref<{GATHERED_BF16}xbf16>, i32, i32) attributes {{link_with = "{experiment_dir}/q4nx_chunk_accum_down.o"}}
    func.func private @q4nx_down_flush_output(memref<{OUT_BF16}xbf16>, i32) attributes {{link_with = "{experiment_dir}/q4nx_chunk_accum_down.o"}}
    func.func private @swiglu_fused(memref<{OUT_BF16}xbf16>, memref<{OUT_BF16}xbf16>, memref<{INTER_BF16}xbf16>, i32) attributes {{link_with = "{experiment_dir}/swiglu_fused.o"}}

    // === Core programs ===
{core_programs}
    // === Memtile DMAs ===
{memtile_dmas}
    // === Core DMAs ===
{core_mems}
    // === Runtime sequence ===
{runtime_sequence}
  }}
}}
"""


if __name__ == "__main__":
    print(generate_mlir())
