"""Test: 2 cores → memtile using PACKET-SWITCHED flows.

Packet flows share a single physical track, with packet headers routing
data to different destinations. This avoids the circuit-switch routing
conflict that causes 2 simultaneous core→memtile flows to deadlock.

Core0 sends with packet_id=0 → memtile S2MM ch3 (offset 0)
Core1 sends with packet_id=1 → memtile S2MM ch4 (offset 32)
"""
...
def generate_mlir() -> str:
  ...
  aie.device(npu2) {{
    ...
    // PACKET-SWITCHED flows for core→memtile
    aie.packet_flow(0) {{
    ...
    aie.packet_flow(1) {{
      ...
      aie.packet_dest<%mt0, DMA : 4>
    }}

    // Circuit-switched flow for memtile→shim (output)
    ...
    // Core0 DMA: MM2S ch1 with packet header (packet_id=0)
    ...
    // Core1 DMA: MM2S ch1 with packet header (packet_id=1)
    ...
    %memtile_dma = aie.memtile_dma(%mt0) {{
      // S2MM ch3 (odd, BD 24): receive packet_id=0 from core0 → offset 0
    ...
    ^inter_r0:
      ...
      // S2MM ch4 (even, BD 1): receive packet_id=1 from core1 → offset 32
    ^inter_r1_start:
