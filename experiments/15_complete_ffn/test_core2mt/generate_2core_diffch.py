"""Test: 2 cores → memtile, using DIFFERENT MM2S channels.

Core0 uses MM2S ch0 (instead of ch1)
Core1 uses MM2S ch1

Hypothesis: the router assigns both to the same southbound track when both
use DMA:1. Using different source channels may force different tracks.
"""
...
def generate_mlir() -> str:
  ...
  aie.device(npu2) {{
    ...
    // KEY CHANGE: core0 uses DMA:0 (MM2S ch0), core1 uses DMA:1 (MM2S ch1)
    aie.flow(%core0, DMA : 0, %mt0, DMA : 3)        // core0 MM2S ch0 → memtile S2MM ch3
    aie.flow(%core1, DMA : 1, %mt0, DMA : 4)        // core1 MM2S ch1 → memtile S2MM ch4
    ...
    // Core0 DMA: MM2S ch0 (not ch1!)
    %mem_core0 = aie.mem(%core0) {{
      %0 = aie.dma_start(MM2S, 0, ^send_bd, ^end)
    ^send_bd:
