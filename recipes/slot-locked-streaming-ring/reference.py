"""CPU reference for the slot-locked streaming-ring recipe."""
        ...
        for patch in range(PATCHES_PER_COLUMN):
            for chunk in range(K_CHUNKS):
                for row_in_patch in range(2):
                    ...
                    parts.append(pack_stream_chunk(scales, zeros, int4))
...
def packed_chunk_matvec(packed_chunk: np.ndarray, act: np.ndarray) -> np.ndarray:
