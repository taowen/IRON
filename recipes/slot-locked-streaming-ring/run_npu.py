#!/usr/bin/env python3
"""Run a slot-locked streaming-ring recipe on real NPU."""
...
def compile_kernel() -> None:
    ...
    src = EXPERIMENT_DIR / "streaming_kernels.cc"
    obj = EXPERIMENT_DIR / "streaming_kernels.o"
    cmd = [
        str(clang),
        "-O2",
        "-std=c++20",
    ...
    print("  Compiling streaming_kernels.cc...")
    run_command(cmd)
...
def check_mlir_structure(mlir_text: str) -> list[str]:
    ...
    expected_counts = {
        "aie.flow(": len(MAIN_COLUMNS) * (ROWS_PER_COLUMN * 2 + 2),
        ...
        "func.call @packed_chunk_accum_slice": expected_tiles * 2,
...
def run_on_npu() -> bool:
    print("=" * 78)
    print("Recipe: Slot-Locked Streaming Ring")
    print("=" * 78)
    print(f"  main columns: {MAIN_COLUMNS}, rows per column: {ROWS_PER_COLUMN}")
    print(f"  K={K}, K_chunks={K_CHUNKS}")
    print(f"  packed stream chunk: {CHUNK_BF16 * 2} bytes")
    print(f"  host stream descriptor: {PATCH_BF16 * 2} bytes (0x{PATCH_BF16 * 2:x})")
    ...
    print("  Preparing packed stream payload and CPU reference...")
    ...
    if mismatch.size == 0:
        print(
            "  PASS: host stream descriptors feed slot-locked ping/pong rings and worker inputs."
        )
        return True
