"""Generate raw MLIR-AIE for a slot-locked streaming ring."""
...
{_lock_pair(tile, "output", 4)}
    ...
    %{tile}_core = aie.core(%{tile}) {{
      ...
      scf.for %chunk = %c0 to %num_chunks step %c1 {{
        ...
        aie.use_lock(%{tile}_act_full, AcquireGreaterEqual, 1)
        scf.if %is_pong {{
          func.call @packed_chunk_accum_slice(%{tile}_wt_pong, %{tile}_act_pong, %m_i32)
            : (memref<{CHUNK_BF16}xbf16>, memref<{ACT_SLICE_BF16}xbf16>, i32) -> ()
        }} else {{
          func.call @packed_chunk_accum_slice(%{tile}_wt_ping, %{tile}_act_ping, %m_i32)
            : (memref<{CHUNK_BF16}xbf16>, memref<{ACT_SLICE_BF16}xbf16>, i32) -> ()
        }}
...
def _runtime_sequence() -> str:
    ...
    for group, column in enumerate(MAIN_COLUMNS):
        ...
        lines += [
            ...
            _npu_push_queue(column, "S2MM", 0, 13, issue_token=True),
        ]
...
{chr(10).join(flows)}
    ...
    func.func private @edge_make_activation_slice(memref<{ACT_SLICE_BF16}xbf16>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/streaming_kernels.o"}}
    func.func private @packed_chunk_accum_slice(memref<{CHUNK_BF16}xbf16>, memref<{ACT_SLICE_BF16}xbf16>, i32) attributes {{link_with = "{experiment_dir}/streaming_kernels.o"}}
    func.func private @flush_projection_output(memref<{OUT_RECORD_BF16}xbf16>, i32, i32, i32) attributes {{link_with = "{experiment_dir}/streaming_kernels.o"}}
