...
constexpr int PACKED_K_CHUNK = 256;
...
void packed_chunk_accum_slice(
    bfloat16 *packed_chunk,
    bfloat16 *activation_slice,
    int32_t num_rows
) {
    constexpr int groups_per_row = PACKED_K_CHUNK / GROUP_SIZE;
    ...
    for (int row = 0; row < num_rows; row++) {
        ...
        for (int group = 0; group < groups_per_row; group++) {
            ...
            uint4 *group_ptr = data + row * (PACKED_K_CHUNK / 2) + group * (GROUP_SIZE / 2);
