"""Generate raw MLIR-AIE for row1 fanout and packet-gather routing."""
...
def _runtime_sequence(context_len: int) -> str:
    ...
    for group in range(NUM_KV_GROUPS):
        query_offset = group * QUERY_GROUP_DWORDS * 4
        lines += [
            _npu_writebd(group, 0, QUERY_GROUP_DWORDS, query_offset),
            _npu_address_patch(group, 0, 0, query_offset),
        ...
        lines += [
            _npu_writebd(group, 15, OUTPUT_GROUP_DWORDS, output_offset),
            _npu_address_patch(group, 15, 2, output_offset),
            _npu_push_queue(group, "S2MM", 0, 15, issue_token=True),
        ]
    for group in range(NUM_KV_GROUPS):
        lines.append(_npu_sync(group))
    lines.append("    }")
    return "\n".join(lines)
...
def _memtile_buffers_and_locks(column: int) -> str:
    prefix = f"mem{column}"
    return f"""
    %{prefix}_query = aie.buffer(%{prefix}) {{sym_name = "{prefix}_query"}} : memref<{QUERY_GROUP_DWORDS}xf32>
    ...
    %{prefix}_query_empty = aie.lock(%{prefix}, 8) {{init = {Q_HEADS_PER_GROUP} : i32, sym_name = "{prefix}_query_empty"}}
    %{prefix}_query_full = aie.lock(%{prefix}, 9) {{init = 0 : i32, sym_name = "{prefix}_query_full"}}
...
def _worker_mem(column: int, row: int) -> str:
    ...
    return f"""    %{prefix}_mem = aie.mem(%{prefix}) {{
      %0 = aie.dma_start(S2MM, 0, ^query_in, ^out_start)
    ^query_in:
      aie.use_lock(%{prefix}_query_empty, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_query : memref<{HEAD_DIM}xf32>, 0, {HEAD_DIM}) {{bd_id = 0 : i32, next_bd_id = 1 : i32}}
...
def _query_and_kv_out_channel(channel: int, column: int, bds: tuple[int, int, int, int, int]) -> str:
    prefix = f"mem{column}"
    query_bd, k_ping_bd, v_ping_bd, k_pong_bd, v_pong_bd = bds
    query_offset = channel * HEAD_DIM
    dims = (
    ...
    return f"""    ^out{channel}_start:
      %out{channel}_dma = aie.dma_start(MM2S, {channel}, ^out{channel}_query, ^out{channel + 1}_start)
    ^out{channel}_query:
      aie.use_lock(%{prefix}_query_full, AcquireGreaterEqual, 1)
      aie.dma_bd(%{prefix}_query : memref<{QUERY_GROUP_DWORDS}xf32>, {query_offset}, {HEAD_DIM}) {{bd_id = {query_bd} : i32, next_bd_id = {k_ping_bd} : i32}}
      aie.use_lock(%{prefix}_query_empty, Release, 1)
...
def _memtile_dma(column: int) -> str:
    ...
    outputs = "\n".join(_query_and_kv_out_channel(ch, column, channel_bds[ch]) for ch in range(Q_HEADS_PER_GROUP))
    return f"""    %{prefix}_dma = aie.memtile_dma(%{prefix}) {{
      %0 = aie.dma_start(S2MM, 0, ^query_in, ^v_in_start)
    ^query_in:
      aie.use_lock(%{prefix}_query_empty, AcquireGreaterEqual, {Q_HEADS_PER_GROUP})
      aie.dma_bd(%{prefix}_query : memref<{QUERY_GROUP_DWORDS}xf32>, 0, {QUERY_GROUP_DWORDS}) {{bd_id = 0 : i32, next_bd_id = 1 : i32}}
      aie.use_lock(%{prefix}_query_full, Release, {Q_HEADS_PER_GROUP})
...
def generate_mlir(context_len: int) -> str:
    ...
    for col in range(NUM_KV_GROUPS):
        ...
        for row in range(Q_HEADS_PER_GROUP):
            ...
            flows.append("    }")
        flows.append(f"    aie.flow(%mem{col}, DMA : 5, %shim{col}, DMA : 0)")
    flow_defs = "\n".join(flows)
    ...
    for col in range(NUM_KV_GROUPS):
        for row in range(Q_HEADS_PER_GROUP):
            ...
            mems.append(_worker_mem(col, row))
    core_defs = "\n\n".join(cores)
    worker_mem_defs = "\n\n".join(mems)
