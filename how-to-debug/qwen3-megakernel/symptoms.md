...
- [Attention Scores Run But Head Layout Is Wrong](#attention-scores-run-but-head-layout-is-wrong)
...
## Attention Scores Run But Head Layout Is Wrong
...
attn_weights look close to uniform
```

Evidence found:

```text
The first failing score checkpoint had 216 score mismatches, exactly
8 odd heads * 27 valid positions, which localized the issue to the second
Q head in each GQA pair rather than to K-cache DMA coverage.
```

Current status:

```text
The score/softmax checkpoint compiles and runs, but numeric verification is not
accepted yet. The next diagnostic should drain or checksum qk_pair so the first
wrong boundary is proven before changing the score math.
```
