...
def main() -> None:
    ...
    if result.errors == 0 and result.qwen3_errors == 0:
        ...
        print(
            ...
            "Workers consume lane-local phase streams loaded from real Qwen3 "
            "weights, phase 0 carries hidden/norm metadata in the lane packet "
            "and computes real row-sharded Q outputs, attention chunk phases "
            "0/1 compute real K/V "
            "projection shards, attention chunk phases 2/3 are drained "
            "diagnostic packet slots, score/PV phases "
            "compute real chunked online attention context for all 16 heads "
            "using two sequential head groups per lane, o_proj is split into "
            "32 row chunks and each chunk computes full cross-fabric partial "
            "contributions through fixed source and target reducer Workers "
            "before materializing the full attention residual vector, "
            ...
            f"chunks and emits {op.ffn_reduce_group_count} 32-row FFN partial "
            "groups through the reducer fabric, "
            "down_proj computes real layer residual shards while reading "
            f"the first {op.ffn_npu_rows} FFN hidden rows reduced from the "
            "preceding gate_up phases and host-packed remaining rows, "
