...
def d1_fixed_cache_attention_context(
    dev,
    max_seq_len,
    q_heads,
    kv_heads,
    head_dim,
    chunk_size,
    kernel_object="d1_fixed_attention.o",
):
    ...
    init_kernel = Kernel(
        "d1_fixed_attention_init_f32",
        kernel_object,
        [state_ty, acc_ty, np.int32],
    )
    update_kernel = Kernel(
        "d1_fixed_attention_update_packed_bf16",
        kernel_object,
        [q_ty, packed_chunk_ty, state_ty, acc_ty, np.int32, np.int32, np.int32],
    )
    finalize_kernel = Kernel(
        "d1_fixed_attention_finalize_into_full_bf16",
        kernel_object,
        [state_ty, acc_ty, out_ty, np.int32, np.int32],
    )

    q_fifo = ObjectFifo(q_ty, name="d1_attn_q", depth=1)
    packed_fifo = ObjectFifo(packed_chunk_ty, name="d1_attn_packed_cache", depth=1)
    out_fifo = ObjectFifo(out_ty, name="d1_attn_context", depth=1)
