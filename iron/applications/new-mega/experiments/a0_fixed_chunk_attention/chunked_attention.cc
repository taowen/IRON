...
#ifndef FIXED_CHUNK_HEAD_DIM
#define FIXED_CHUNK_HEAD_DIM 128
#endif

#ifndef FIXED_CHUNK_SIZE
#define FIXED_CHUNK_SIZE 64
#endif

#ifndef FIXED_CHUNK_ATTN_SCALE
#define FIXED_CHUNK_ATTN_SCALE 0.08838834764831845f
#endif
...
void fixed_chunk_attention_init_f32(float *__restrict state, float *__restrict acc, int32_t head_dim)
{
    event0();

    state[0] = -__builtin_inff();
    state[1] = 0.0f;
...
void fixed_chunk_attention_update_packed_bf16(const bfloat16 *__restrict q,
                                              ...
                                              int32_t head_dim)
{
    ...
    for (int32_t row = 0; row < chunk_size; row++) {
        ...
        float score = aie::reduce_add(dot.template to_vector<float>()) * FIXED_CHUNK_ATTN_SCALE;
    ...
    for (int32_t row = 0; row < chunk_size; row++) {
        ...
        for (int32_t dim = 0; dim < head_dim; dim++) {
            acc[dim] += weight * static_cast<float>(v_row[dim]);
        }
    }

    state[0] = new_max;
    state[1] = old_sum * correction + chunk_sum;

    event1();
}

void fixed_chunk_attention_finalize_bf16(const float *__restrict state,
                                         ...
                                         int32_t head_dim)
{
    ...
    if (denom <= 0.0f) {
        ...
        for (int32_t dim = 0; dim < head_dim; dim++) {
            out[dim] = static_cast<bfloat16>(acc[dim] * inv);
        }
    }
